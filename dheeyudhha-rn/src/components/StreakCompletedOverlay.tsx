  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none" className="items-center justify-center z-50">
      ...
      <Animated.View
        style={[
          styles.card,
          { opacity, transform: [{ scale }, { translateY }], backgroundColor: isDark ? '#0f172a' : '#ffffff' },
        ]}
      >
        <View className="absolute inset-0 items-center justify-center opacity-30">
          <View className="w-48 h-48 bg-orange-500 rounded-full" />
        </View>
        
        <View className="items-center justify-center relative z-10 p-8">
          <Flame size={80} color="#f97316" fill="#f97316" />
          <Text style={{ color: isDark ? '#ffffff' : '#0f172a', fontSize: 28, fontWeight: '900', marginTop: 16, textAlign: 'center' }}>
            Streak{'\n'}Completed! 🔥
          </Text>
          <Text style={{ color: isDark ? '#fed7aa' : '#ea580c', fontWeight: '700', marginTop: 8, fontSize: 18, textAlign: 'center' }}>
