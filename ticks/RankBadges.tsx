...
const BadgeContainer = ({ children, label, glowColor }: { children: React.ReactNode, label: string, glowColor: string }) => (
  <span className="relative inline-flex items-center justify-center group ml-1 select-none" title={label}>
    {/* Animated glow effect */}
    <span className="absolute inset-0 rounded-full opacity-30 group-hover:opacity-70 group-hover:blur-[6px] transition-all duration-500 animate-pulse" style={{ backgroundColor: glowColor, filter: 'blur(3px)' }}></span>
    <div className="relative z-10 transform transition-all duration-500 group-hover:scale-125 group-hover:-translate-y-1 drop-shadow-[0_4px_6px_rgba(0,0,0,0.3)]">
      {children}
    </div>
  </span>
);
