...
const SYLLABUS = [
  { class_grade: '6', subject: 'Mathematics', chapters: ['Knowing Our Numbers', 'Whole Numbers', 'Playing With Numbers'] },
  { class_grade: '7', subject: 'Mathematics', chapters: ['Integers', 'Fractions and Decimals', 'Data Handling'] },
  { class_grade: '8', subject: 'Mathematics', chapters: ['Rational Numbers', 'Linear Equations in One Variable', 'Understanding Quadrilaterals'] },
  { class_grade: '9', subject: 'Mathematics', chapters: ['Number Systems', 'Polynomials', 'Coordinate Geometry'] },
  { class_grade: '10', subject: 'Mathematics', chapters: ['Real Numbers', 'Polynomials', 'Pair of Linear Equations in Two Variables'] }
];
...
async function generateQuestionsFromOpenRouter(chapterName, classGrade, subject, modelId) {
  const prompt = `Generate 10 high-quality, hard-difficulty academic multiple-choice questions for Class ${classGrade} ${subject}, specifically focusing on the chapter "${chapterName}".
Each question must have exactly 4 options and indicate the correct option index (0 to 3). Include a helpful "hint" to guide the student, and a detailed "explanation" for why the answer is correct. Make all the questions hard difficulty and ensure they are accurate and relevant to the NCERT curriculum.
...
Format of array items:
{
  ...
  const timeoutId = setTimeout(() => controller.abort(), 240000); // 4 minute timeout for 10 questions
...
async function run() {
  console.log('\n=======================================================');
  console.log(' 🚀 Dheeyudha — Bulk AI Question Seeder (Maths)');
  console.log('=======================================================\n');
        ...
        try {
          ...
          for (const q of questions) {
            const { error } = await adminClient.from('questions').insert({
              ...
              chapter:       chapterName,
              points:        3, // User requested 3 points
              time_limit:    5, // User requested 5 minutes
              difficulty:    'hard', // User requested hard difficulty
              options:       q.options,
