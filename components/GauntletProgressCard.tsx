      ...
      <AnimatePresence>
        {copied && (
          <motion.div
            ...
            className="fixed top-5 left-1/2 -translate-x-1/2 z-[99999] bg-slate-900 text-white text-sm font-bold px-5 py-3 rounded-2xl shadow-xl flex items-center gap-2"
          >
        ...
        <div className="relative overflow-hidden rounded-3xl bg-indigo-950 p-6 flex flex-col justify-between border-2 border-indigo-500/20 shadow-xl shadow-indigo-900/10 group cursor-pointer active:scale-[0.98] transition-transform">
          ...
          <div className="relative z-10 flex items-start justify-between">
            <div className="flex-1 min-w-0 pr-3">
              ...
              <h3 className="text-xl font-black text-white leading-tight mb-1">{CHAPTER}</h3>
            ...
            <button
              onClick={handleShare}
              className="w-11 h-11 rounded-2xl bg-indigo-500/20 border border-indigo-400/30 flex items-center justify-center shrink-0 hover:bg-indigo-500/40 active:scale-95 transition-all"
          ...
          <div className="relative z-10 mt-5 bg-slate-900/40 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1 text-sm font-bold text-white">
                <BookOpen className="w-4 h-4 text-indigo-400" />
                Level {Math.min(level, TOTAL_LEVELS)} of {TOTAL_LEVELS}
              </div>
              <div className="font-extrabold text-indigo-300 text-sm">{pct}%</div>
            </div>
            ...
            {!isCompleted ? (
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1 text-xs font-black text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-md border border-emerald-400/20">
                  <span>✓</span> Completed with Mastery
                </div>
                <div className="text-white text-xs font-bold uppercase tracking-wide flex items-center gap-0.5 group-hover:text-indigo-300 transition-colors">
