...
const Header: React.FC<HeaderProps> = ({ isMobile = false }) => {
  ...
  const UserAvatar: React.FC = () => {
    ...
    const avatarUrl = cachedMeta?.custom_avatar_url || cachedMeta?.avatar_url || user?.user_metadata?.custom_avatar_url || user?.user_metadata?.avatar_url || user?.user_metadata?.picture;
                ...
                <Link href="/profile" className="active:scale-95 transition-transform flex items-center justify-center">
                  <span className="inline-block h-9 w-9 rounded-full overflow-hidden border-[2px] border-slate-200 dark:border-slate-700 shadow-sm relative">
                    {(user?.user_metadata?.custom_avatar_url || user?.user_metadata?.avatar_url || user?.user_metadata?.picture) ? (
                      <Image src={user.user_metadata.custom_avatar_url || user.user_metadata.avatar_url || user.user_metadata.picture} alt="avatar" fill className="object-cover" referrerPolicy="no-referrer" />
                    ) : (
