        ...
        <>
            ...
            {currentStatus === "pending" && !selfMarked && (
                <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 sm:p-8 border border-gray-100 dark:border-slate-800 shadow-sm">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-2">Did you get it right?</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm mb-5">
                        Compare honestly with the model answer above. Claim your{" "}
                        <strong className="text-violet-700 dark:text-violet-400">{questionPoints} points</strong> if you&apos;re correct.
                    </p>

                    <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-xl mb-5 flex gap-3">
            ...
            {(selfMarked || (submission.self_marked_correct && ["pending_check"].includes(currentStatus))) && (
                <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 sm:p-8 border border-emerald-100 dark:border-emerald-800/50 shadow-sm">
                    <div className="flex items-center gap-4 mb-5">
                        <div className="w-14 h-14 bg-emerald-100 dark:bg-emerald-900/40 rounded-2xl flex items-center justify-center shrink-0">
                            <CheckCircle2 className="w-7 h-7 text-emerald-600 dark:text-emerald-400" />
                        </div>
                        <div>
                            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Points Awarded!</h2>
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl space-y-2 text-sm text-slate-600 dark:text-slate-300 mb-4">
                    ...
                    {selfMarkResult && (
                        <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 rounded-xl flex items-center justify-between mb-4">
                            <span className="text-sm text-emerald-700 dark:text-emerald-400 font-semibold">Your total points</span>
                            <span className="text-2xl font-black text-emerald-700 dark:text-emerald-400">{selfMarkResult.newTotal}</span>
                        </div>
                    )}
            ...
            {currentStatus === "ai_confirmed_wrong" && (
                <div className="bg-red-50 dark:bg-red-900/20 rounded-2xl border border-red-200 dark:border-red-800/50 p-6 text-center mt-5">
                    <XCircle className="w-12 h-12 text-red-500 mx-auto mb-3" />
                    <h3 className="text-xl font-black text-red-700 dark:text-red-300">Answer Confirmed Incorrect</h3>
                    <p className="text-red-600 dark:text-red-400 mt-2 text-sm mb-4">
                        Points + {questionPoints >= 15 ? "3-point penalty" : "penalty"} have been deducted.
                    </p>
                    <div className="flex flex-col gap-3 max-w-sm mx-auto">
                        {!challengeId && (
                            <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800/50 p-4 rounded-2xl mb-1 text-sm text-indigo-800 dark:text-indigo-300 text-left">
                        ...
                        <a
                            ...
                            className="w-full inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white dark:bg-slate-800 border border-red-200 dark:border-red-800/50 rounded-xl text-red-700 dark:text-red-400 font-bold text-sm hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors shadow-sm"
                        >
            ...
            {(currentStatus === "auto_approved" || currentStatus === "ai_confirmed_correct") && (
                <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl border border-emerald-200 dark:border-emerald-800/50 p-6 text-center mt-5">
                    ...
                    <p className="text-emerald-600 dark:text-emerald-400 mt-2 text-sm mb-4">
                    ...
                    {currentStatus === "ai_confirmed_correct" && (
                        <a
                            href={`/submission/${submission.id}/ai-review`}
                            className="inline-flex items-center gap-2 px-6 py-2.5 bg-white dark:bg-slate-800 border border-emerald-200 dark:border-emerald-800/50 rounded-xl text-emerald-700 dark:text-emerald-400 font-bold text-sm hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors shadow-sm"
                        >
