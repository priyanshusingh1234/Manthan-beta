...
import { Gift, CheckCircle, X, Shield, Star, Coins } from 'lucide-react';
...
export default function LoginBonusModal({ onClose }: { onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden relative" onClick={e => e.stopPropagation()}>
                <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-slate-800 dark:hover:text-white transition-colors z-10">
                    <X size={20} />
                </button>
                ...
                <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-8 text-center relative overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 -mb-4 -ml-4 w-24 h-24 bg-indigo-300 opacity-20 rounded-full blur-xl"></div>
                    <Gift size={48} className="mx-auto text-white mb-3" />
                    <h2 className="text-2xl font-black text-white tracking-tight">7-Day Login Bonus</h2>
                    <p className="text-indigo-100 font-medium mt-1">Claim your daily reward to grow faster!</p>
                </div>
...
function DayCard({ day, currentDay, claimedToday, isLarge = false }: { day: number, currentDay: number, claimedToday: boolean, isLarge?: boolean }) {
    ...
    return (
        <div className={`relative flex flex-col items-center justify-center p-3 rounded-2xl border-2 transition-all ${
            isClaimed 
            ...
            {day === 7 ? (
                ...
                <div className="flex items-center gap-1">
                    ...
                    <span className={`text-lg font-black ${isClaimed ? '' : isNext ? 'text-slate-800 dark:text-white' : ''}`}>
                        {day * 5}
                    </span>
                </div>
            )}
            
            {day === 7 && <div className="text-[10px] font-bold text-center mt-1 leading-tight text-indigo-600 dark:text-indigo-400">Pioneer<br/>Badge</div>}
        </div>
    );
}
