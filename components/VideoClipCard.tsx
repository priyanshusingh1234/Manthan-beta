...
export default function VideoClipCard({ post, currentUserId, onUpdate, onCommentClick, compact = true }: VideoClipCardProps) {
    ...
    return (
        <div className={`relative bg-black overflow-hidden mx-auto ${compact ? 'rounded-[2.5rem] aspect-[9/16] w-full max-w-[340px] shadow-lg border border-slate-100 dark:border-white/5' : 'h-full w-full flex items-center shadow-2xl overflow-visible'}`}>
            ...
            {/* Top Toolbar */}
            <div className="absolute top-[max(1rem,env(safe-area-inset-top))] right-4 flex flex-col gap-4 z-10">
            ...
            <div className="absolute bottom-[max(2rem,env(safe-area-inset-bottom))] right-4 flex flex-col items-center gap-7 z-20">
                {compact && (
                    <Link href={singlePostUrl} className="flex flex-col items-center gap-1.5 group">
                        <div className="w-14 h-14 rounded-full bg-violet-600 shadow-[0_10px_30px_rgba(139,92,246,0.5)] flex items-center justify-center group-hover:scale-110 active:scale-90 transition-all border-2 border-white/20">
                ...
                <div className="flex flex-col items-center gap-1.5">
                    <button onClick={handleLike} className="w-14 h-14 rounded-full bg-black/20 backdrop-blur-md flex items-center justify-center border border-white/10 hover:bg-white/10 active:scale-90 transition-all">
                        <Heart className={`w-7 h-7 ${isLiked ? 'fill-rose-500 text-rose-500' : 'text-white'}`} />
                    </button>
                    {likesCount > 0 && <span className="text-white text-[13px] font-black drop-shadow-xl">{likesCount}</span>}
                </div>

                <div className="flex flex-col items-center gap-1.5">
                    <button 
                        onClick={(e) => { e.stopPropagation(); if (onCommentClick) onCommentClick(); else if (compact) window.location.href = singlePostUrl; }} 
                        className="w-14 h-14 rounded-full bg-black/20 backdrop-blur-md flex items-center justify-center border border-white/10 hover:bg-white/10 active:scale-90 transition-all"
                    >
                        <MessageCircle className="w-7 h-7 text-white" />
                    </button>
                    {commentsCount > 0 && <span className="text-white text-[13px] font-black drop-shadow-xl">{commentsCount}</span>}
                </div>

                <button onClick={handleShare} className="w-14 h-14 rounded-full bg-black/20 backdrop-blur-md flex items-center justify-center border border-white/10 hover:bg-white/10 active:scale-90 transition-all">
                    <Share2 className="w-7 h-7 text-white" />
                </button>

                {!compact && (
                    <button onClick={handleDownload} className="w-14 h-14 rounded-full bg-black/20 backdrop-blur-md flex items-center justify-center border border-white/10 hover:bg-white/10 text-white active:scale-90 transition-all">
                        <Download className="w-6 h-6" />
                    </button>
                )}
            </div>

            {/* Author & Content Info (Bottom Left) */}
            <div className="absolute bottom-[max(2rem,env(safe-area-inset-bottom))] left-5 right-20 z-20 pointer-events-none mb-1">
                ...
                {post.content && (
                    <div className="pointer-events-auto pr-6">
                        <p className="text-[15px] text-white font-bold drop-shadow-2xl leading-relaxed line-clamp-3 scrollbar-none transition-all">
                            {post.content}
                        </p>
                    </div>
                )}
