...
export default function CompleteProfileOverlay({ onComplete }: { onComplete?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-slate-50 dark:bg-slate-950 flex flex-col items-center p-0 sm:p-8 animate-in fade-in zoom-in duration-300 overflow-y-auto">
      {/* Dynamic Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 via-purple-500/10 to-pink-500/10 dark:from-indigo-900/20 dark:via-purple-900/20 dark:to-pink-900/20 pointer-events-none fixed" />
      
      <div className="w-full max-w-md bg-white dark:bg-slate-900 h-full sm:h-auto sm:rounded-[2.5rem] shadow-2xl p-8 sm:p-10 relative z-10 border-0 sm:border border-slate-200 dark:border-slate-800 flex flex-col justify-center min-h-[100dvh] sm:min-h-0 pt-[calc(2rem+env(safe-area-inset-top))] pb-[calc(2rem+env(safe-area-inset-bottom))]">
        <div className="flex-1 flex flex-col justify-center sm:block">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 mt-4 bg-gradient-to-r from-indigo-600 to-fuchsia-600 text-white font-black text-xs uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-indigo-500/30 active:scale-95 transition-all flex justify-center items-center gap-2 mb-8"
            >
