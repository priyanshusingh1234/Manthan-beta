...
async function broadcast() {
    ...
    for (const user of users) {
        await createNotification({
            userId: user.id,
            type: 'system',
            title: '✨ Grand Poetry Competition!',
            body: 'Show us your poetic brilliance! Huge rewards are waiting. Tap here to read the rules and participate! 🎉',
            href: '/poetry-competition'
        });
        sent++;
        if (sent % 10 === 0) console.log(`Sent ${sent}/${users.length}`);
    }
    
    console.log(`Push Broadcast complete! Sent to ${sent} users.`);
    process.exit(0);
}
