...
const supabaseUrl = process.env.EXPO_PUBLIC_SUPABASE_URL || 'https://127.0.0.1'
...
if (!process.env.EXPO_PUBLIC_SUPABASE_URL || !process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY) {
  ...
  try {
    ...
    console.error('❌ Invalid EXPO_PUBLIC_SUPABASE_URL format');
  }
}
...
const realSupabaseUrl = process.env.EXPO_PUBLIC_SUPABASE_URL || 'https://127.0.0.1';
const isBrowser = typeof window !== 'undefined';
const isNative = isBrowser && !!(window as any).Capacitor?.isNativePlatform?.();
const proxyBaseUrl = isNative 
  ? (process.env.EXPO_PUBLIC_APP_URL || 'https://manthan-beta-c975.vercel.app')
