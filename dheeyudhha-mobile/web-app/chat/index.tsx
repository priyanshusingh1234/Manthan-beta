...
export default function ChatListPage() {
  ...
  return (
    <View className="flex flex-col min-h-screen bg-slate-50 dark:bg-slate-950" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      {/* Header — matches app style */}
      <View className="sticky top-0 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200/60 dark:border-slate-800/60" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
        <View className="px-4 pt-4 pb-3">
          <View className="flex items-center justify-between mb-4 flex-row">
            <View className="flex items-center gap-3 flex-row">
              <View onPress={() => router.push('/feed')} className="h-9 w-9 flex items-center justify-center rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 active:scale-95 transition-transform flex-row">
                <ArrowLeft className="w-4 h-4" />
              </View>
              <Text className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Messages</Text>
            </View>
            <View className="h-9 w-9 bg-indigo-600 text-white rounded-full flex items-center justify-center shadow-lg shadow-indigo-600/30 active:scale-95 transition-transform flex-row">
          ...
          <View className="relative">
            ...
            />
            {searchQuery && <View onPress={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400"><X className="h-4 w-4" /></View>}
          </View>
        </View>
      </View>
      ...
      <View className="flex-1 flex-row">
        ...
        {searchQuery && isSearching ? (
          <View className="flex justify-center py-16 flex-row"><Loader2 className="w-6 h-6 text-indigo-500 animate-spin" /></View>
        ) : searchQuery && searchResults.length > 0 ? (
          <View>
            <View className="px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest text-indigo-500 bg-indigo-50/50 dark:bg-indigo-900/10">Global Results</View>
            {searchResults.map(res => (
              <View key={res.id} onPress={() => startChat(res.id, res.full_name)} className="w-full flex items-center gap-3 px-4 py-3.5 active:bg-slate-100 dark:active:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800/50 text-left flex-row">
                <View className="h-11 w-11 rounded-full bg-slate-200 dark:bg-slate-800 relative overflow-hidden shrink-0">
                  {(res.avatar_url || res.avatar) ? <Image source={{ uri: res.avatar_url || res.avatar }} alt="Avatar" fill className="object-cover" /> : <View className="h-full w-full flex items-center justify-center font-bold text-indigo-600 dark:text-indigo-400 text-lg flex-row">{(res.full_name || res.name)?.[0]?.toUpperCase()}</View>}
                </View>
                <View className="flex-1 min-w-0 flex-row">
                  ...
                  <Text className="text-sm text-slate-500 dark:text-slate-400 truncate mt-0.5">@{res.username || 'scholar'}</Text>
                </View>
              </View>
            ))}
          </View>
          ...
          <View>
            {loading ? (
              Array.from({ length: 6 }).map((_, i) => (
                <View key={i} className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-100 dark:border-slate-800/50 flex-row">
                  <View className="h-12 w-12 rounded-full bg-slate-200 dark:bg-slate-800 animate-pulse shrink-0" />
                  <View className="flex-1 space-y-2 flex-row"><View className="h-3.5 bg-slate-200 dark:bg-slate-800 rounded animate-pulse w-2/3" /><View className="h-3 bg-slate-100 dark:bg-slate-700 rounded animate-pulse w-1/2" /></View>
                </View>
              ))
            ) : rooms.length === 0 ? (
              <View className="flex flex-col items-center justify-center py-24 px-8 text-center">
                <View className="w-20 h-20 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center mb-5 transform rotate-6 flex-row">
                  <MessageCirclePlus className="w-10 h-10 text-indigo-500 -rotate-6" />
                </View>
                <Text className="text-lg font-bold text-slate-900 dark:text-white mb-1">No conversations yet</Text>
                <Text className="text-sm text-slate-500 dark:text-slate-400">Search for a scholar above to start chatting.</Text>
              </View>
            ) : (
              <View className="animate-in fade-in duration-300">
                {filteredRooms.filter(r => r.status === 'pending').length > 0 && (
                  <View className="mb-4">
                     <View className="px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest text-indigo-500 bg-indigo-50/50 dark:bg-indigo-900/10">Message Requests</View>
                     {filteredRooms.filter(r => r.status === 'pending').map((room, idx) => (
                       <ChatCard key={room.id} room={room} onPress={() => router.push(`/chat/${room.id}`)} user={user} index={idx} />
                     ))}
                  </View>
                )}
                ...
                {filteredRooms.filter(r => r.status !== 'pending').length > 0 && (
                   <>
                    <View className="px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Recent Chats</View>
                    {filteredRooms.filter(r => r.status !== 'pending').map((room, idx) => (
                      <ChatCard key={room.id} room={room} onPress={() => router.push(`/chat/${room.id}`)} user={user} index={idx} />
                    ))}
                  </>
...
const ChatCard = memo(function ChatCard({ room, onClick, user, index = 0 }: { room: ChatRoom; onClick: () => void; user: any; index?: number }) {
  ...
  return (
    <View 
      onPress={onClick} 
      className="w-full flex items-center gap-3 px-4 py-3.5 active:bg-slate-100 dark:active:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800/50 text-left animate-in fade-in slide-in-from-bottom-2 duration-300 flex-row"
      style={{ animationFillMode: 'both', animationDelay: `${index * 40}ms` }}
    >
      <View className="relative shrink-0">
        <View className="h-12 w-12 rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden">
          {room.participant.avatar_url
            ? <Image source={{ uri: room.participant.avatar_url }} alt={room.participant.full_name} fill className="object-cover" sizes="48px" />
            : <View className="h-full w-full flex items-center justify-center font-bold text-indigo-600 dark:text-indigo-400 text-lg flex-row">{room.participant.full_name?.[0]?.toUpperCase() || 'U'}</View>
          }
        </View>
      </View>
      <View className="flex-1 min-w-0 flex-row">
        <View className="flex items-center justify-between mb-0.5 flex-row">
          <BadgedName name={room.participant.full_name} userId={room.participant.user_id} nameClassName={`text-[15px] truncate pr-2 ${isUnread ? 'font-extrabold text-slate-900 dark:text-white' : 'font-semibold text-slate-800 dark:text-slate-200'}`} className="flex-1 min-w-0 flex-row" />
          <Text className={`text-[11px] shrink-0 font-semibold ${isUnread ? 'text-indigo-500' : 'text-slate-400'}`}>{timeLabel}</Text>
        </View>
        <View className="flex items-center justify-between gap-2 flex-row">
          <Text className={`text-[13px] truncate flex-1 ${isUnread ? 'font-semibold text-slate-800 dark:text-slate-200' : 'text-slate-500 dark:text-slate-400'}`}>
            {room.last_message ? formatPreview(room.last_message.content, room.last_message.message_type) : 'Tap to start chatting'}
          </Text>
          <View className="flex items-center gap-1.5 shrink-0 flex-row">
            {room.last_message && room.last_message.sender_id === user?.id && (
              <Text className={`flex items-center gap-0.5 ${room.last_message.is_read ? 'text-indigo-500' : 'text-slate-400'}`}>
                {room.last_message.is_read ? <CheckCheck className="w-3.5 h-3.5" strokeWidth={2.5} /> : <Check className="w-3.5 h-3.5" strokeWidth={2.5} />}
                <Text className="text-[9px] font-bold uppercase tracking-widest">{room.last_message.is_read ? 'Seen' : 'Sent'}</Text>
              </Text>
            )}
            {isUnread && <View className="w-2.5 h-2.5 bg-indigo-500 rounded-full shadow shadow-indigo-500/40" />}
          </View>
        </View>
      </View>
    </View>
