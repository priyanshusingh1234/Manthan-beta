...
async function vibrate(style: 'light' | 'medium' = 'light') {
  if (typeof navigator !== 'undefined' && navigator.vibrate) {
    navigator.vibrate(style === 'light' ? 30 : 60);
  }
}
...
const MessageItem = memo(function MessageItem({
  ...
  if (msg.content.startsWith('__CALL_ENDED__') || msg.content.startsWith('__CALL_STARTED__')) {
    ...
    return (
      <View className="flex justify-center my-2 px-4 flex-row">
        <View className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-semibold px-4 py-1.5 rounded-full flex items-center gap-1.5 flex-row">
  ...
  return (
    <View className="w-full flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-300">
      {showDate && (
        <View className="flex justify-center my-3 flex-row">
          <Text className="px-3 py-1 bg-slate-200/80 dark:bg-slate-800 rounded-lg text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
      ...
      >
        {/* Swipe reply hint */}
        {!isMe && swipeX > 20 && (
          <View className="absolute left-2 opacity-70">
            <Reply className="w-5 h-5 text-indigo-500" />
          </View>
        )}
        {isMe && swipeX < -20 && (
          <View className="absolute right-2 opacity-70">
        ...
        {/* Avatar for others */}
        {!isMe && (
          <View className="w-7 h-7 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden shrink-0 self-end mb-0.5">
            {participant?.avatar_url
              ? <Image source={{ uri: participant.avatar_url }} alt="" className="object-cover w-full h-full" />
              : <View className="w-full h-full flex items-center justify-center text-indigo-600 dark:text-indigo-400 text-xs font-bold flex-row">{participant?.full_name?.[0]?.toUpperCase()}</View>
            }
          </View>
        )}
        ...
        >
          {replyAuthor && (
            <View className={`mb-1 px-3 py-1.5 rounded-xl text-[12px] border-l-[3px] max-w-full ${isMe ? 'bg-indigo-500/10 border-indigo-400' : 'bg-slate-100 dark:bg-slate-700 border-slate-400'}`}>
              <Text className={`font-bold truncate ${isMe ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-700 dark:text-slate-300'}`}>{replyAuthor}</Text>
              <Text className="truncate text-slate-500 dark:text-slate-400">{replyPreview}</Text>
            </View>
          )}
          <View
          ...
          >
            {msg.message_type === 'image_once' ? (
              <View 
                className="flex items-center gap-2 px-4 py-3 cursor-pointer select-none active:bg-black/5 transition-colors flex-row"
                onPress={(e) => {
                  e.stopPropagation();
                  onImageClick(msg.content, msg.message_type, msg.id, isMe);
                }}
              >
                <View className={`flex items-center justify-center w-8 h-8 rounded-full ${isMe ? 'bg-white/20' : 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400'}`}>
                  <ImageIcon className="w-4 h-4" />
                </View>
                <View className="flex flex-col">
                  <Text className={`font-bold text-sm leading-none ${isMe ? 'text-white' : 'text-slate-900 dark:text-white'}`}>Photo</Text>
                  <Text className={`text-[10px] font-semibold mt-0.5 opacity-80 ${isMe ? 'text-white/80' : 'text-slate-500'}`}>View once</Text>
                </View>
              </View>
              ...
              >
                <Image source={{ uri: msg.content }} alt="Image" fill className="object-cover" />
              </View>
            ) : (
              <View className="flex flex-col">
                <Text className="px-3.5 py-2 text-[14.5px] leading-[1.5] whitespace-pre-wrap break-words">
                  {mainContent}
                  {meta.edited && <Text className="text-[10px] opacity-70 italic ml-1.5 font-medium">(edited)</Text>}
                </Text>
                {(() => {
                  const urlMatch = mainContent.match(CHAT_LINK_PREVIEW_REGEX);
                  const previewUrl = urlMatch ? urlMatch[0] : null;
                  return previewUrl ? (
                    <View className="px-2 pb-2 pt-0 w-full max-w-[280px]">
          ...
          {meta.reactions && Object.keys(meta.reactions).length > 0 && (
            <View className={`flex gap-1 mt-1 px-1 ${isMe ? 'justify-end' : 'justify-start'} max-w-[200px] flex-wrap`}>
              {Object.entries(meta.reactions).map(([emoji, users]) => (
                <View key={emoji} onPress={() => onReaction(msg.id, emoji)} className={`rounded-full px-2 py-0.5 text-[11px] flex items-center gap-1 border shadow-sm cursor-pointer select-none active:scale-95 transition-transform ${((users as string[]).includes(user?.id)) ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/30 dark:border-indigo-800' : 'bg-white border-slate-200 dark:bg-slate-800 dark:border-slate-700'}`}>
                  <Text>{emoji}</Text>
                  <Text className={`font-bold ${((users as string[]).includes(user?.id)) ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'}`}>{Array.isArray(users) ? users.length : 1}</Text>
                </View>
              ))}
            </View>
          )}
          <View className={`flex items-center gap-1 mt-0.5 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
            <Text className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">{timeStr}</Text>
            {isMe && (msg.is_read
              ? <View className="flex items-center gap-0.5 flex-row" title="Read"><CheckCheck className="w-3.5 h-3.5 text-indigo-500" strokeWidth={2.5} /><Text className="text-[9px] text-indigo-500 font-bold uppercase tracking-widest">Seen</Text></View>
              : <View className="flex items-center gap-0.5 flex-row" title="Sent"><Check className="w-3 h-3 text-slate-400" strokeWidth={2.5} /><Text className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Sent</Text></View>
            )}
          </View>
        </View>
      </View>
...
function ChatRoomContent() {
  ...
  return (
    <View
      ...
      onPress={() => { if (showHeaderMenu) setShowHeaderMenu(false); }}
    >
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <View
      ...
      >
        {isSelectionMode ? (
          <View className="flex items-center gap-2 px-3 h-14 flex-row">
            <View onPress={() => { setIsSelectionMode(false); setSelectedIds([]); }} className="p-2 rounded-full active:bg-slate-100 dark:active:bg-slate-800">
              <X className="w-5 h-5 text-slate-700 dark:text-slate-200" />
            </View>
            <Text className="flex-1 font-bold text-[16px] text-slate-900 dark:text-white flex-row">{selectedIds.length} selected</Text>
            <View
              onPress={() => { if (contextMsg) { setReplyingTo(contextMsg); setIsSelectionMode(false); setSelectedIds([]); setTimeout(() => inputRef.current?.focus(), 100); } }}
              className="p-2 rounded-full active:bg-slate-100 dark:active:bg-slate-800 text-slate-600 dark:text-slate-300"
            >
              <Reply className="w-5 h-5" />
            </View>
            <View
              onPress={() => setShowMultiDeleteSheet(true)}
          ...
          <View className="flex items-center gap-2 px-3 h-14 flex-row">
            <View onPress={() => router.push('/chat')} className="p-2 -ml-1 rounded-full active:bg-slate-100 dark:active:bg-slate-800 shrink-0">
            ...
            <View
              ...
              onPress={() => participant?.username && router.push(`/user/${participant.username}`)}
            >
              {participant?.avatar_url
                ? <Image source={{ uri: participant.avatar_url }} alt="" className="object-cover w-full h-full" />
                : <View className="w-full h-full flex items-center justify-center font-bold text-indigo-600 dark:text-indigo-400 text-base flex-row">{displayName[0]?.toUpperCase()}</View>
              }
            </View>
            <View className="flex-1 min-w-0 cursor-pointer flex-row" onPress={() => participant?.username && router.push(`/user/${participant.username}`)}>
              ...
              <Text className="text-[11px] text-slate-400 font-medium">
                {isTyping ? <Text className="text-indigo-500 font-bold animate-pulse">Typing...</Text> : isOnline ? '🟢 Online' : 'Tap for info'}
              </Text>
            </View>
            <View onPress={() => callCtx.startCall(roomId, 'voice', participant?.user_id, participant?.full_name || 'Scholar')} className="p-2 rounded-full active:bg-slate-100 dark:active:bg-slate-800 text-slate-600 dark:text-slate-300 shrink-0">
              <Phone className="w-5 h-5" />
            </View>
            <View onPress={() => callCtx.startCall(roomId, 'video', participant?.user_id, participant?.full_name || 'Scholar')} className="p-2 rounded-full active:bg-slate-100 dark:active:bg-slate-800 text-slate-600 dark:text-slate-300 shrink-0">
              <Video className="w-5 h-5" />
            </View>
            <View className="relative shrink-0">
              <View
                onPress={(e) => { e.stopPropagation(); setShowHeaderMenu(v => !v); }}
                className="p-2 rounded-full active:bg-slate-100 dark:active:bg-slate-800 text-slate-600 dark:text-slate-300"
              >
                <MoreVertical className="w-5 h-5" />
              </View>
              {showHeaderMenu && (
                <View className="absolute right-0 top-11 w-56 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
                  {participant?.username && (
                    <View onPress={() => { setShowHeaderMenu(false); router.push(`/user/${participant.username}`); }}
                      className="w-full text-left px-4 py-3.5 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-3 active:bg-slate-100 flex-row">
                      View Profile
                    </View>
                  )}
                  <View onPress={() => { setShowHeaderMenu(false); setIsSelectionMode(true); }}
                    className="w-full text-left px-4 py-3.5 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-3 active:bg-slate-100 flex-row">
                    Select Messages
                  </View>
                  <View
                    onPress={() => { setShowHeaderMenu(false); setShowClearChatConfirm(true); }}
                    className="w-full text-left px-4 py-3.5 text-sm font-semibold text-rose-500 hover:bg-red-50 dark:hover:bg-rose-900/10 flex items-center gap-3 active:bg-red-50 border-t border-slate-100 dark:border-slate-800 flex-row"
                  >
      ...
      {isBlocked && (
        <View className="bg-rose-50 dark:bg-rose-950/30 px-4 py-2 flex items-center justify-center gap-2 border-b border-rose-100 dark:border-rose-900/50 flex-row">
          <Ban className="w-3.5 h-3.5 text-rose-500" />
          <Text className="text-[11px] font-bold text-rose-500 uppercase tracking-tight">You have blocked this scholar</Text>
        </View>
      )}

      {/* ── In-page incoming call banner (user already on this chat page) ── */}
      {incomingCallBanner && (
        <View className="shrink-0 bg-slate-900 text-white px-4 py-3 flex items-center justify-between gap-3 animate-in slide-in-from-top duration-300 flex-row">
          <View className="flex items-center gap-3 min-w-0 flex-row">
            <View className="w-9 h-9 rounded-full bg-indigo-600 flex items-center justify-center shrink-0 flex-row">
              {incomingCallBanner.type === 'video' ? <Video className="w-5 h-5" /> : <Phone className="w-5 h-5" />}
            </View>
            <View className="min-w-0">
              <Text className="font-bold text-sm truncate">{participant?.full_name || 'Scholar'}</Text>
              <Text className="text-xs text-white/60">Incoming {incomingCallBanner.type} call...</Text>
            </View>
          </View>
          <View className="flex items-center gap-2 shrink-0 flex-row">
            <View
              onPress={() => {
                setIncomingCallBanner(null);
                incomingCallBannerRef.current = null;
              ...
              className="h-10 w-10 rounded-full bg-red-500 flex items-center justify-center active:scale-95 transition-transform flex-row"
            >
            ...
            <View
              ...
              className="h-10 w-10 rounded-full bg-green-500 flex items-center justify-center active:scale-95 transition-transform flex-row"
            >
              <Phone className="w-5 h-5" />
            </View>
          </View>
      ...
      {/* ── Messages ───────────────────────────────────────────────────────── */}
      <View
        className="flex-1 overflow-y-auto py-2 space-y-0.5 flex-row"
        style={{ WebkitOverflowScrolling: 'touch' }}
        onPress={() => { if (showContextSheet) setShowContextSheet(false); if (showHeaderMenu) setShowHeaderMenu(false); }}
      >
        {loading ? (
          <View className="flex flex-col gap-3 px-4 py-4 w-full">
            {/* Incoming skeleton */}
            <View className="flex gap-2 flex-row">
              <View className="w-7 h-7 rounded-full bg-slate-200 dark:bg-slate-800 animate-pulse shrink-0 self-end" />
              <View className="bg-slate-100 dark:bg-slate-800 rounded-2xl rounded-bl-sm p-4 w-[60%] animate-pulse" />
            </View>
            {/* Outgoing skeleton */}
            <View className="flex flex-row-reverse gap-2 mt-2">
              <View className="bg-indigo-100 dark:bg-indigo-900/40 rounded-2xl rounded-br-sm p-4 w-[50%] animate-pulse" />
            </View>
            {/* Short Outgoing skeleton */}
            <View className="flex flex-row-reverse gap-2 mt-1">
              <View className="bg-indigo-100 dark:bg-indigo-900/40 rounded-2xl rounded-br-sm p-3 w-[30%] animate-pulse" />
            </View>
            {/* Incoming skeleton */}
            <View className="flex gap-2 mt-4 flex-row">
              <View className="w-7 h-7 rounded-full bg-slate-200 dark:bg-slate-800 animate-pulse shrink-0 self-end" />
              <View className="bg-slate-100 dark:bg-slate-800 rounded-2xl rounded-bl-sm p-5 w-[75%] animate-pulse" />
            </View>
            {/* Outgoing skeleton */}
            <View className="flex flex-row-reverse gap-2 mt-2">
              <View className="bg-indigo-100 dark:bg-indigo-900/40 rounded-2xl rounded-br-sm p-3 w-[45%] animate-pulse" />
            </View>
          </View>
        ) : messages.length === 0 ? (
          <View className="flex flex-col items-center justify-center h-full gap-3 px-8 text-center">
            <View className="w-16 h-16 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl flex items-center justify-center flex-row">
          ...
          <>
            {hasMore && (
              <View className="flex justify-center py-4 flex-row">
                <View
                  onPress={loadMoreMessages}
                  disabled={loadingMore}
                  className="px-4 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 text-[11px] uppercase tracking-wider font-bold text-slate-600 dark:text-slate-300 active:scale-95 transition-transform disabled:opacity-50 flex items-center gap-2 flex-row"
                >
        ...
        <View ref={messagesEndRef} className="h-2" />
      </View>

      {/* ── Context sheet (long press) ────────────────────────────────────── */}
      {showContextSheet && contextMsg && (
        <>
          <View className="fixed inset-0 bg-black/20 dark:bg-black/50 z-40 backdrop-blur-sm" onPress={() => setShowContextSheet(false)} />
          <View className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-slate-900 rounded-t-3xl shadow-2xl animate-in slide-in-from-bottom duration-200 overflow-hidden flex flex-col">
            <View className="w-10 h-1 bg-slate-200 dark:bg-slate-700 rounded-full mx-auto mt-3 mb-3 shrink-0" />
            
            {/* Reaction picker */}
            <View className="px-5 pb-3 pt-1 flex justify-between items-center shrink-0 border-b border-slate-100 dark:border-slate-800 flex-row">
              {['❤️', '😂', '🔥', '👍', '😢', '😮'].map(emoji => (
                <View
                  key={emoji}
                  onPress={() => {
            ...
            {/* Message preview */}
            <View className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 shrink-0">
              <Text className="text-[13px] text-slate-500 dark:text-slate-400 truncate">
                {contextMsg.content.includes('|||META|||') ? contextMsg.content.split('|||META|||')[0].slice(0, 60) : contextMsg.content.slice(0, 60)}
              </Text>
            </View>
            <View className="overflow-y-auto max-h-[50vh] shrink-0">
            {[
              ...
              <View key={label} onPress={action} className={`w-full flex items-center gap-4 px-5 py-4 active:bg-slate-50 dark:active:bg-slate-800 ${label === 'Delete' ? 'border-t border-slate-100 dark:border-slate-800' : ''}`}>
                <View className={`w-10 h-10 rounded-full ${label === 'Delete' ? 'bg-rose-50 dark:bg-rose-900/20' : label === 'Reply' ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'bg-slate-100 dark:bg-slate-800'} flex items-center justify-center`}>
      ...
      {showMultiDeleteSheet && (
        <>
          <View className="fixed inset-0 bg-black/20 dark:bg-black/50 z-40 backdrop-blur-sm" onPress={() => setShowMultiDeleteSheet(false)} />
          <View className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-slate-900 rounded-t-3xl shadow-2xl animate-in slide-in-from-bottom duration-200 overflow-hidden">
            <View className="w-10 h-1 bg-slate-200 dark:bg-slate-700 rounded-full mx-auto mt-3 mb-3" />
            <View className="px-5 pb-3 border-b border-slate-100 dark:border-slate-800">
              <Text className="text-[13px] font-bold text-slate-700 dark:text-slate-300 text-center">Delete {selectedIds.length} Option{selectedIds.length > 1 ? 's' : ''}</Text>
            </View>
            ...
            <View 
              ...
              className="w-full flex items-center gap-4 px-5 py-4 active:bg-slate-50 dark:active:bg-slate-800 flex-row"
            >
              <View className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center flex-row">
                <Trash2 className="w-5 h-5 text-slate-700 dark:text-slate-300" />
              </View>
              <Text className="font-semibold text-slate-700 dark:text-slate-300">Delete for Me</Text>
            </View>
            ...
            {selectedIds.filter(id => messages.find(m => m.id === id)?.sender_id === user?.id).length === selectedIds.length && (
              <View 
                ...
                className="w-full flex items-center gap-4 px-5 py-4 active:bg-slate-50 dark:active:bg-slate-800 border-t border-slate-100 dark:border-slate-800 flex-row"
              >
                <View className="w-10 h-10 rounded-full bg-rose-50 dark:bg-rose-900/20 flex items-center justify-center flex-row">
                  <Trash2 className="w-5 h-5 text-rose-500" />
                </View>
                <Text className="font-semibold text-rose-500">Delete for Everyone</Text>
              </View>
            )}
      ...
      {/* ── Send error banner ───────────────────────────────────────── */}
        {sendError && (
          <View className="flex items-center gap-2 px-4 py-2 bg-rose-50 dark:bg-rose-950/30 border-b border-rose-100 dark:border-rose-900/30 flex-row">
            <Text className="text-xs font-bold text-rose-600 dark:text-rose-400 flex-1 flex-row">{sendError}</Text>
            <View onPress={() => setSendError(null)} className="text-rose-400 text-xs font-bold">✕</View>
          </View>
        )}
        {/* Reply preview */}

        {replyingTo && (
          <View className="flex items-center gap-3 px-4 pt-2.5 pb-0 flex-row">
            <View className="flex-1 border-l-[3px] border-indigo-500 pl-3 py-1 bg-indigo-50 dark:bg-indigo-900/20 rounded-r-xl min-w-0 flex-row">
              <Text className="text-[11px] font-bold text-indigo-600 dark:text-indigo-400 mb-0.5">
                Replying to {replyingTo.sender_id === user?.id ? 'yourself' : participant?.full_name}
              </Text>
              <Text className="text-[12px] text-slate-500 dark:text-slate-400 truncate">
                {replyingTo.content.includes('|||META|||') ? replyingTo.content.split('|||META|||')[0].slice(0, 50) : replyingTo.content.slice(0, 50)}
              </Text>
            </View>
            <View onPress={() => setReplyingTo(null)} className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 flex-row">
        ...
        {editingMsg && (
          <View className="flex items-center gap-3 px-4 pt-2.5 pb-0 flex-row">
            <View className="flex-1 border-l-[3px] border-indigo-500 pl-3 py-1 bg-indigo-50 dark:bg-indigo-900/20 rounded-r-xl min-w-0 flex items-center gap-2 flex-row">
              <Edit2 className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
              <Text className="text-[12px] font-bold text-indigo-600 dark:text-indigo-400 mb-0">
                Editing Message
              </Text>
            </View>
            <View onPress={() => { setEditingMsg(null); setNewMessage(''); }} className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 flex-row">
              <X className="w-4 h-4 text-slate-500" />
            </View>
          </View>
        )}
        <View className="flex items-end gap-2 px-3 py-2 flex-row">
          {/* Image button */}
          <View className="flex flex-col gap-1 items-center mb-0.5">
              <View
                onPress={() => fileInputRef.current?.click()}
                disabled={uploadingImage}
                className="w-9 h-9 flex items-center justify-center rounded-full text-slate-500 dark:text-slate-400 active:bg-slate-100 dark:active:bg-slate-800 shrink-0 flex-row"
              >
          ...
          <TextInput ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />

          {/* Auto-growing textarea */}
          <View className="flex-1 bg-slate-100 dark:bg-slate-800 rounded-2xl px-4 py-2.5 min-h-[40px] flex-row">
            <textarea
              ref={inputRef}
              rows={1}
              value={newMessage}
          ...
          <View
            ...
            className="w-10 h-10 bg-indigo-600 disabled:bg-slate-200 dark:disabled:bg-slate-700 text-white disabled:text-slate-400 rounded-full flex items-center justify-center shadow-md shadow-indigo-600/20 disabled:shadow-none active:scale-90 transition-all shrink-0 mb-0.5 flex-row"
          >
      ...
      {roomStatus?.status === 'pending' && (
          <View className="absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-4 pb-8 z-50 animate-in slide-in-from-bottom">
              {roomStatus.created_by === user?.id ? (
                  <View className="text-center">
                      <Text className="font-bold text-slate-900 dark:text-white mb-1">Message Request Sent</Text>
                  ...
                  <View>
                      <Text className="font-bold text-slate-900 dark:text-white mb-1 text-center">Message Request</Text>
                      ...
                      <View className="flex items-center gap-3 flex-row">
                          <View
                              ...
                              className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold active:scale-95 transition-transform flex-row"
                          >
                              Accept
                          </View>
                          <View
                              ...
                              className="flex-1 py-3 bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl font-bold active:scale-95 transition-transform flex-row"
                          >
                              Decline
                          </View>
                      </View>
      ...
      {showClearChatConfirm && (
        <View
          className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/50 backdrop-blur-sm flex-row"
        ...
        >
          ...
          >
            <View className="flex justify-center mb-5 flex-row">
              <View className="w-14 h-14 rounded-full bg-rose-100 dark:bg-rose-900/30 flex items-center justify-center flex-row">
                <Trash2 className="w-7 h-7 text-rose-500" />
              </View>
            </View>
            <Text className="text-center text-lg font-black text-slate-900 dark:text-white mb-1">Clear Chat?</Text>
            <Text className="text-center text-sm text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
              Your messages will be <Text>permanently deleted</Text> for everyone.<br />
              Their messages will be hidden <Text>only for you</Text>.
            </Text>
            <View
              onPress={clearChat}
...
export default function ChatRoomPage() {
  return (
    <React.Suspense fallback={
      <View className="h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 flex-row">
