...
function QuestionBreakdown({ snapshot }: { snapshot: AttemptSnapshot[] }) {
    ...
    return (
        <View className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-xl overflow-hidden">
            <View className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3 flex-row">
                <BarChart3 className="w-5 h-5 text-indigo-500" />
                <Text className="font-black uppercase tracking-widest text-sm text-slate-700 dark:text-slate-200">Full Question Breakdown</Text>
            </View>
                    ...
                    return (
                        <View key={idx}>
                            <View
                                ...
                                className="w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex-row"
                            >
                                {/* Status dot */}
                                <View className={`shrink-0 w-2 h-2 rounded-full ${statusColor}`} />
                                {/* Q number */}
                                <Text className="shrink-0 w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-black text-[10px] text-slate-500 flex-row">
                                    {idx + 1}
                                </Text>
                                {/* Question text */}
                                <Text className="flex-1 text-sm font-medium leading-snug text-slate-700 dark:text-slate-300 text-left line-clamp-2 flex-row">
                                    {q.title}
                                </Text>
                            ...
                            {isOpen && (
                                <View className="px-5 pb-5 space-y-2">
                                    {(q.options || []).map((opt, oIdx) => {
                                        ...
                                        return (
                                            <View key={oIdx} className={`flex items-center gap-3 p-3 rounded-xl border text-xs ${cls}`}>
                                                <Text className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center font-black text-[10px] ${isCorrectOpt ? 'bg-emerald-500 text-white' : isWrong ? 'bg-red-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500'}`}>
                                                    {String.fromCharCode(65 + oIdx)}
                                                </Text>
                                                <Text className="flex-1 leading-snug flex-row">{opt}</Text>
                                    ...
                                    {!hasAnswer && (
                                        <Text className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-2">— Not attempted</Text>
                                    )}
                                </View>
                            )}
                        </View>
...
function ArenaPage({ params }: { params: { id: string } }) {
    ...
    // ── LOADING ──
    if (loading) return (
        <View className="min-h-[100dvh] bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center">
            <View className="w-12 h-12 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mb-4" />
            <Text className="font-bold text-indigo-600 dark:text-indigo-400 animate-pulse tracking-widest uppercase text-sm">Initializing Gauntlet...</Text>
        </View>
    );
    ...
    if (error || !gauntlet) return (
        <View className="min-h-[100dvh] bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6 flex-row">
            <View className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 p-8 rounded-3xl max-w-md w-full text-center">
                <ShieldAlert className="w-12 h-12 text-red-500 mx-auto mb-4" />
                <Text className="text-xl font-black mb-2 text-red-600 dark:text-red-400">Gauntlet Unavailable</Text>
                <Text className="text-slate-500 text-sm mb-6">{error || 'Not found'}</Text>
                <View onPress={() => router.push('/tests')} className="w-full py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold rounded-xl">Return to Arena</View>
            </View>
        </View>
    );
    ...
    if (isSubmitted) {
        ...
        return (
            <View className="min-h-[100dvh] bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white pb-32">
                {/* ── Sticky header bar ── */}
                <View 
                    className="sticky top-0 z-40 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 px-4 py-3"
                    style={{ paddingTop: 'calc(env(safe-area-inset-top) + 0.75rem)' }}
                >
                    <View className="max-w-2xl mx-auto flex items-center justify-between gap-3 flex-row">
                        <View onPress={() => router.push('/tests')} className="text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-indigo-500 transition-colors">← Arena</View>
                        <Text className="text-[10px] font-black uppercase tracking-widest text-slate-400 truncate">{gauntlet.title}</Text>
                        <View
                            onPress={handleShare}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex-row"
                        >
                ...
                <View className="max-w-2xl mx-auto p-4 space-y-6 pt-6">
                    {/* ── Hero Score Card ── */}
                    <View className={`relative bg-gradient-to-br ${gauntlet.color} rounded-[2.5rem] p-6 sm:p-8 overflow-hidden shadow-2xl`}>
                        <View className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 75% 20%, white 0%, transparent 60%)' }} />
                        <View className="relative">
                            <View className="flex items-center gap-2 mb-5 flex-row">
                                <Trophy className="w-5 h-5 text-yellow-300" />
                                <Text className="text-[10px] font-black uppercase tracking-[0.2em] text-white/80">Gauntlet Complete</Text>
                            </View>
                            <View className="flex items-end gap-3 mb-6 flex-row">
                                <Text className="text-7xl font-black italic tracking-tighter text-white leading-none">{totalScore}</Text>
                                <View className="pb-2">
                            ...
                            <View className="grid grid-cols-4 gap-2">
                                {[
                                    ...
                                    <View key={stat.label} className="bg-white/10 backdrop-blur-sm rounded-2xl p-3 text-center">
                            ...
                            <View className="mt-5">
                                <View className="h-2 bg-white/20 rounded-full overflow-hidden">
                                    <View className="h-full bg-emerald-400 rounded-full transition-all duration-700" style={{ width: `${acc}%` }} />
                                </View>
                                <View className="flex justify-between mt-1 flex-row">
                    ...
                    {/* ── Bonus Points Banner ── */}
                    {bonusMessage && (
                        <View className="flex items-center gap-3 p-4 bg-emerald-50 dark:bg-emerald-500/10 border-2 border-emerald-400 dark:border-emerald-500/50 rounded-2xl flex-row">
                            <View className="w-10 h-10 rounded-xl bg-emerald-500 flex items-center justify-center shrink-0 shadow-lg shadow-emerald-500/30 flex-row">
                                <CheckCircle2 className="w-5 h-5 text-white" />
                            </View>
                            <View>
                                <Text className="font-black text-sm text-emerald-700 dark:text-emerald-400">{bonusMessage}</Text>
                                <Text className="text-[10px] text-emerald-600/70 dark:text-emerald-500/60 font-bold mt-0.5">Bonus points added to your profile instantly</Text>
                            </View>
                        </View>
                    )}
                    ...
                    {resultSnap.length > 0 ? (
                        ...
                        <View className="bg-white dark:bg-slate-900 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl p-8 text-center">
                    ...
                    <View className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-xl overflow-hidden">
                        <View className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3 flex-row">
                            ...
                            <Text className="font-black uppercase tracking-widest text-sm text-slate-700 dark:text-slate-200">Hall of Fame</Text>
                            <Text className="ml-auto text-[10px] font-black uppercase tracking-widest text-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 px-3 py-1 rounded-full">{gauntlet.title}</Text>
                        </View>
                    ...
                    {/* ── Action Buttons ── */}
                    <View className="flex gap-3 pb-8 flex-row">
                        <View
                        ...
                        <View
                            onPress={() => router.push('/')}
                            className="flex-1 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl font-black uppercase text-xs tracking-widest active:scale-95 transition-all flex-row"
                        >
                            Return Home
                        </View>
                    </View>
    ...
    return (
        <View className="h-[100dvh] bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white flex flex-col overflow-hidden">
            {/* Header */}
            <View 
                className="shrink-0 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 p-3 z-50"
                style={{ paddingTop: 'calc(env(safe-area-inset-top) + 0.75rem)' }}
            >
                <View className="max-w-4xl mx-auto flex items-center justify-between gap-3 flex-row">
                    {/* Timer */}
                    <View className={`px-3 py-2 rounded-xl font-black text-sm flex items-center gap-2 tabular-nums ${timeLeft < 300 ? 'bg-red-100 dark:bg-red-500/10 text-red-600 animate-pulse' : 'bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300'}`}>
                        <Clock className="w-4 h-4" />
                        <Text>{String(mins).padStart(2, '0')}:{String(secs).padStart(2, '0')}</Text>
                    </View>
                    {/* Progress */}
                    <View className="flex-1 hidden sm:flex flex-col items-center gap-1">
                        <Text className="text-[10px] font-black uppercase tracking-widest text-slate-400">{gauntlet.title}</Text>
                        <View className="w-full max-w-xs h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                            <View
            ...
            {/* Question */}
            <View className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col pb-24">
                <View className="max-w-3xl mx-auto w-full flex-1 flex flex-col">
                    <View className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm mb-6">
                        <View className="flex items-center justify-between mb-5 flex-row">
                            <Text className="text-[10px] font-black uppercase tracking-widest text-indigo-500">Q {currentIndex + 1} / {questions.length}</Text>
                            <Text className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg">{currentQ.subject}</Text>
                        </View>
                        <Text className="text-base md:text-lg font-semibold leading-relaxed">{currentQ.title}</Text>
                        <View className="mt-6 space-y-2.5">
                            {currentQ.options.map((opt, oIdx) => (
                                ...
                                >
                                    <View className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs shrink-0 ${answers[currentIndex] === oIdx ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500'}`}>
                                        {String.fromCharCode(65 + oIdx)}
                                    </View>
                                    <Text className="font-medium text-sm leading-snug">{opt}</Text>
                                </View>
                            ))}
                        </View>
                    </View>
                    <View className="flex gap-3 mt-auto flex-row">
                        <View
                            onPress={() => setCurrentIndex(c => c - 1)}
                            disabled={currentIndex === 0}
                            className="flex-1 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl font-black text-xs uppercase tracking-widest disabled:opacity-30 active:scale-95 transition-all flex-row"
                        >← Prev</View>
                        <View
                            onPress={() => setCurrentIndex(c => c + 1)}
            ...
            {/* Question Palette */}
            <View 
                className="shrink-0 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 p-2 overflow-x-auto"
                style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 0.5rem)' }}
            >
                <View className="flex gap-1.5 min-w-max px-1 flex-row">
                    {questions.map((_, idx) => (
                        <View
                            key={idx}
