...
import { ArrowLeft, MessageCircle, Send, Loader2, User, X } from 'lucide-react-native';
...
export default function SinglePostClient({ postId }: { postId: string }) {
    ...
    if (loading) {
        return (
            <View className="fixed inset-0 flex items-center justify-center bg-black flex-row">
                <View className="w-10 h-10 border-3 border-violet-500/20 border-t-violet-500 rounded-full animate-spin" />
            </View>
        );
    }
    ...
    if (!post) {
        return (
            <View className="min-h-[100dvh] flex items-center justify-center bg-white dark:bg-slate-950 flex-row">
                <Text className="text-slate-400 font-bold">Post not found.</Text>
            </View>
        );
    }
    ...
    if (post.video_url) {
        return (
            <View className="fixed inset-0 bg-black z-[100] flex flex-col md:flex-row overflow-hidden">
                ...
                <View className="relative flex-1 h-[100dvh] flex items-center justify-center bg-black flex-row">
                    <View className="w-full h-full max-w-[450px] aspect-[9/16] relative">
                        <VideoClipCard
                            post={post}
                ...
                <View className="hidden md:flex flex-col w-[400px] border-l border-white/10 bg-[#0a0a0a] h-full">
                    <View className="p-6 border-b border-white/5 bg-white/[0.02]">
                        <Text className="text-lg font-black text-white flex items-center gap-2 flex-row">
                            <MessageCircle className="w-5 h-5 text-violet-400" />
                            Discussion
                            <Text className="text-xs bg-violet-600 px-2 py-0.5 rounded-full ml-auto">{post.comments_count || 0}</Text>
                        </Text>
                    </View>
                    <View className="flex-1 overflow-y-auto p-4 space-y-4 flex-row">
                        {comments.map((c: any) => (
                            <View key={c.id} className="flex gap-3 flex-row">
                                <View className="w-9 h-9 rounded-full overflow-hidden bg-white/5 border border-white/10 shrink-0">
                                    {c.author?.avatar_url ? <Image src={c.author.avatar_url} className="w-full h-full object-cover" /> : <View className="w-full h-full flex items-center justify-center text-white/30 text-[10px] flex-row">{(c.author?.name || 'U')[0]}</View>}
                                </View>
                                <View className="flex-1 flex-row">
                                    <View className="bg-white/5 rounded-2xl p-3 border border-white/[0.03]">
                                        <Text className="font-black text-white text-[13px] mb-0.5">{c.author?.name || 'Scholar'}</Text>
                                        <Text className="text-white/70 text-[14px] leading-relaxed">{c.content}</Text>
                                    </View>
                                    <Text className="text-[10px] text-white/30 font-bold mt-1 px-1">{formatDistanceToNow(new Date(c.created_at), { addSuffix: true })}</Text>
                                </View>
                            </View>
                        ))}
                    </View>
                    {currentUserId && (
                        <View onPress={submitComment} className="p-4 border-t border-white/5">
                            <View className="relative flex items-center flex-row">
                                ...
                                />
                                <View type="submit" disabled={!newComment.trim()} className="absolute right-2 p-2 bg-violet-600 rounded-xl text-white">
                ...
                <View className="md:hidden">
                    ...
                    <View
                        ...
                        className="fixed bottom-[max(2rem,env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 z-[105] bg-white/10 backdrop-blur-xl border border-white/20 text-white rounded-full px-6 py-2.5 font-black text-sm flex items-center gap-2 active:scale-95 transition-transform flex-row"
                    >
                    ...
                    {showComments && (
                        <View className="fixed inset-0 z-[200] bg-black/40 backdrop-blur-[2px] flex items-end animate-in fade-in duration-300 flex-row" onPress={() => setShowComments(false)}>
                            <View
                            ...
                            >
                                <View className="w-12 h-1.5 bg-white/10 rounded-full mx-auto mt-2 mb-6 shrink-0" onPress={() => setShowComments(false)} />
                                <View className="flex items-center justify-between mb-6 shrink-0 flex-row">
                                    ...
                                    <View onPress={() => setShowComments(false)} className="p-2 -mr-2 bg-white/5 rounded-full text-white/40"><X className="w-5 h-5" /></View>
                                </View>
                                        ...
                                        <View key={c.id} className="flex gap-3 flex-row">
                                            ...
                                            <View className="flex-1 min-w-0 flex-row">
                                                <View className="bg-white/5 rounded-[1.25rem] rounded-tl-none p-4">
                                                    ...
                                                    <Text className="text-white/80 text-[15px] leading-relaxed break-words">{c.content}</Text>
                                                </View>
                                                <Text className="text-[10px] text-white/20 font-black mt-1 px-1 uppercase tracking-wider">{formatDistanceToNow(new Date(c.created_at), { addSuffix: true })}</Text>
                                            </View>
                                        </View>
                                    ))}
                                    {comments.length === 0 && (
                                ...
                                {currentUserId && (
                                    <View onPress={submitComment} className="relative flex items-center shrink-0 flex-row">
                                        <TextInput
                                            value={newComment}
                                            onChange={e => setNewComment(e.target.value)}
                                        ...
                                        />
                                        <View type="submit" disabled={!newComment.trim()} className="absolute right-2.5 p-3 bg-violet-600 rounded-xl text-white shadow-lg active:scale-90 transition-transform">
    ...
    return (
        <View className="min-h-[100dvh] bg-white dark:bg-slate-950 flex flex-col">
            <View 
                className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800/60 px-4 py-3 flex items-center gap-4 text-slate-900 dark:text-white flex-row"
                style={{ paddingTop: 'calc(env(safe-area-inset-top) + 0.75rem)' }}
            >
                <View onPress={() => router.back()} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors">
                    <ArrowLeft className="w-5 h-5" />
                </View>
                <Text className="font-black text-lg tracking-tight">Discussion</Text>
            </View>

            <View 
                className="flex-1 w-full max-w-3xl mx-auto px-4 pb-[calc(env(safe-area-inset-bottom)+2rem)] flex-row"
                style={{ paddingTop: 'calc(env(safe-area-inset-top) + 4.5rem)' }}
            >
                <View className="mb-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
                    <View className="flex items-center gap-3 mb-2 flex-row">
                        <Link href={profileUrl}>
                            <View className="relative w-11 h-11 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-800 shrink-0">
                        ...
                        <View className="flex flex-col min-w-0">
                            ...
                            />
                            <Text className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tight">
