...
import { Trophy, Target, Zap, Star, MapPin, GraduationCap, TrendingUp, BookOpen, Users, ChevronRight, Flame } from 'lucide-react-native';
...
export default async function StudentProfilePage({ params }: Props) {
    ...
    if (!username) return (<View className="min-h-screen py-12 flex items-center justify-center text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 flex-row">User not found</View>);
    ...
    try {
        ...
        if (!fetchedUser) {
            return (
                <View className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 flex-row">
                    <View className="bg-white dark:bg-slate-900 p-12 rounded-3xl shadow-lg border border-slate-100 dark:border-slate-800 text-center max-w-md">
                        <View className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6 flex-row">
                            <Text className="text-3xl">👻</Text>
                        </View>
                        <Text className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-2">User Not Found</Text>
                        <Text className="text-slate-500 dark:text-slate-400">The student profile you are looking for doesn&apos;t exist or hasn&apos;t set a username yet.</Text>
                    </View>
                </View>
            );
        }
        ...
        return (
            <View className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20 pt-6 sm:pt-8 w-full">
                {/* Banner Section - Full width on mobile */}
                <View className="max-w-[90rem] mx-auto sm:px-6 lg:px-8">
                    <View className="h-48 sm:h-80 w-full relative sm:bg-slate-800 sm:rounded-[2.5rem] overflow-hidden shadow-2xl ring-1 ring-slate-900/5 transition-all duration-500">
                        {banner ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <Image src={banner} alt="banner" className="w-full h-full object-cover opacity-85 scale-105" />
                        ) : (
                            <View className="absolute inset-0 bg-gradient-to-br from-indigo-500 via-purple-500 to-fuchsia-500 opacity-90" />
                        )}
                        <View className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent sm:via-transparent" />
                    </View>
                </View>
                <View className="max-w-5xl mx-auto sm:px-6 lg:px-8 -mt-16 sm:-mt-28 relative z-10 w-full">
                    {/* Profile Card - Native Look on Mobile */}
                    <View className="bg-white dark:bg-slate-900 rounded-t-[3rem] sm:rounded-3xl shadow-[0_-15px_30px_-5px_rgba(0,0,0,0.1)] sm:shadow-xl p-5 sm:p-10 border-t sm:border border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row gap-6 sm:gap-8 items-center sm:items-start text-center sm:text-left w-full">
                        
                        <View className="relative shrink-0">
                            {meta.cosmetics?.includes('avatar_glow') && (
                                <View className="absolute -inset-2 bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 rounded-full blur-xl opacity-70 animate-pulse transition-opacity -mt-16 sm:-mt-20"></View>
                            )}
                            {avatar ? (
                                // eslint-disable-next-line @next/next/no-img-element
                                <Image src={avatar} alt={name} className={`w-28 h-28 sm:w-40 sm:h-40 rounded-full object-cover shadow-2xl relative -mt-16 sm:-mt-20 bg-white dark:bg-slate-900 transition-transform hover:scale-105 duration-300 ${meta.cosmetics?.includes('avatar_glow') ? 'ring-4 ring-transparent shadow-indigo-500/50' : 'ring-4 ring-white dark:ring-slate-900'}`} />
                            ) : (
                                <View className={`w-28 h-28 sm:w-40 sm:h-40 rounded-full bg-gradient-to-tr from-indigo-100 to-blue-50 dark:from-indigo-900/40 dark:to-blue-900/20 flex items-center justify-center text-5xl font-bold text-indigo-500 dark:text-indigo-400 shadow-2xl relative -mt-16 sm:-mt-20 ${meta.cosmetics?.includes('avatar_glow') ? 'ring-4 ring-transparent shadow-indigo-500/50' : 'ring-4 ring-white dark:ring-slate-900'}`}>{String(name[0] || 'S').toUpperCase()}</View>
                            )}
                            {isOnline && (
                        ...
                        <View className="flex-1 w-full flex-row">
                            <BadgedName 
                                ...
                                className="flex flex-wrap items-center justify-center sm:justify-start gap-2 sm:gap-3 flex-row"
                            />
                            <View className="flex flex-wrap items-center justify-center sm:justify-start gap-3 mt-1 flex-row">
                                <Text className="text-lg font-mono text-indigo-500 dark:text-indigo-400 font-semibold">@{username}</Text>
                                {isOnline && (
                            ...
                            {(() => {
                                ...
                                return (
                                    <View className="mt-3 flex items-center gap-2 max-w-xs mx-auto sm:mx-0 flex-row">
                                        <View
                                            className="flex items-center gap-1 px-2.5 py-1 rounded-full text-white shrink-0 text-xs font-black flex-row"
                                            style={{ background: `linear-gradient(135deg, ${c.from}, ${c.to})` }}
                                        >
                                            ⚡ Lv.{xpLevel.level} · {c.label}
                                        </View>
                                        <View className="flex-1 h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden flex-row">
                                            <View
                                        ...
                                        <Text className="text-[10px] text-slate-400 font-bold shrink-0">{xpLevel.xpInLevel}/50</Text>
                                    </View>
                                );
                            })()}

                            <View className="flex flex-wrap gap-4 mt-4 justify-center sm:justify-start text-sm font-medium text-slate-600 dark:text-slate-400 w-full flex-row">
                                {school && (
                                    <View className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full flex-row">
                            ...
                            {bio && <Text className="mt-5 text-base text-slate-600 dark:text-slate-300 leading-relaxed max-w-2xl mx-auto sm:mx-0 w-full">{bio}</Text>}

                            {/* Streak badge */}
                            {streakCount > 0 && (
                                <Link href={`/user/${username}`}
                                    className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-2xl border transition-all w-fit cursor-default flex-row"
                                    style={{
                                ...
                                >
                                    ...
                                    />
                                    <Text className={`text-sm font-black ${streakGoalMetToday ? 'text-orange-700' : 'text-slate-500 dark:text-slate-400'}`}>
                                    ...
                                    {streakGoalMetToday && (
                                        <Text className="text-[10px] font-black text-orange-500">🔥 Active today</Text>
                                    )}
                                </Link>
                            )}

                            {/* League badge */}
                            <Link href="/league" className="mt-3 inline-flex items-center gap-2.5 px-4 py-2 rounded-2xl border w-fit hover:opacity-80 transition-opacity flex-row"
                                style={{ borderColor: `${userLeague.color}55`, background: `${userLeague.color}12` }}>
                                <Text className="text-lg">🏆</Text>
                                <View>
                                    <Text className="text-xs font-black" style={{ color: userLeague.color }}>{userLeague.name} League</Text>
                                    <Text className="text-[10px] text-slate-400 font-bold">{monthlyPoints} pts this month</Text>
                                </View>
                            </Link>
                        ...
                        <View className="mt-8 sm:mt-0 flex flex-col gap-4 shrink-0 sm:w-[240px] w-full">
                            {/* Learning Profile Analysis */}
                            <View className="bg-white dark:bg-slate-900 border sm:border-slate-100 dark:sm:border-slate-800 p-6 rounded-3xl sm:rounded-[2rem] shadow-sm sm:shadow-md relative overflow-hidden group/analysis border-slate-100 dark:border-slate-800">
                                <View className="absolute top-0 right-0 w-24 h-24 bg-indigo-500/5 rounded-full blur-2xl group-hover/analysis:bg-indigo-500/10 transition-colors"></View>
                                <Text className="text-[10px] font-black uppercase tracking-widest text-indigo-500 dark:text-indigo-400 mb-5 flex items-center gap-2 flex-row">
                                ...
                                <View className="space-y-4 sm:space-y-5">
                                    <View className="flex items-center gap-4 flex-row">
                                        <View className="w-10 h-10 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl flex items-center justify-center shrink-0 flex-row">
                                    ...
                                    <View className="flex items-center gap-4 flex-row">
                                        <View className="w-10 h-10 bg-purple-50 dark:bg-purple-500/10 rounded-xl flex items-center justify-center shrink-0 flex-row">
                            ...
                            <View className="relative group perspective-1000 w-full shadow-2xl rounded-3xl sm:rounded-[2rem]">
                                <View className="absolute -inset-1 bg-gradient-to-r from-amber-400 via-orange-500 to-rose-500 rounded-3xl sm:rounded-[2rem] blur-sm opacity-20 group-hover:opacity-40 transition duration-1000 group-hover:duration-200" />
                                <View className="relative bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700 text-white p-6 rounded-3xl sm:rounded-[2rem] shadow-xl text-center h-full flex flex-col justify-center transform transition-transform group-hover:-translate-y-1">
                                    <View className="flex justify-center mb-4 scale-150 transform transition-transform group-hover:scale-[1.7] group-hover:rotate-12 duration-500 flex-row">
                                        {myRank === 1 ? <GoldBadge /> : myRank === 2 ? <SilverBadge /> : myRank === 3 ? <BronzeBadge /> : <View className="p-4 bg-white/10 rounded-full"><Trophy className="w-10 h-10 text-amber-400" /></View>}
                                    </View>
                                    <View className="text-[10px] items-center justify-center flex gap-1.5 font-bold text-slate-400 bg-black/20 py-1 sm:py-1.5 px-4 rounded-full border border-white/5 mx-auto w-fit flex-row">
                    ...
                    {!isTeacher && (
                        <View className="mt-8 sm:mt-12 w-full">
                            <PublicProfileTabs 
                                userId={fetchedUser.id}
                                username={username}
                                myRank={myRank}
        ...
        return (<View className="min-h-screen py-12 flex items-center justify-center text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 flex-row">Profile rendering failed</View>);
    }
}
