...
export default function LinkPreview({ url }: { url: string }) {
  ...
  if (loading) {
    return (
      <View className="flex items-center gap-2 p-3 mt-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 animate-pulse flex-row">
  ...
  return (
    <Link href={data.link} className="block mt-2 max-w-[280px] group select-none">
      <View className="flex flex-col rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm transition-all hover:shadow-md hover:border-indigo-300 dark:hover:border-indigo-700">
        {data.image && (
          <View className="w-full h-32 relative bg-slate-100 dark:bg-slate-900 overflow-hidden border-b border-slate-100 dark:border-slate-700">
            <Image source={{ uri: data.image }} alt="Preview" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
          </View>
        )}
        <View className="p-3">
          <View className="flex items-center gap-1.5 mb-1.5 flex-row">
          ...
          <Text className="font-bold text-sm text-slate-900 dark:text-white leading-tight mb-1 truncate">
            {data.title}
          </Text>
          <Text className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed">
            {data.description}
          </Text>
          {data.authorName && (
            <View className="flex items-center gap-1.5 mt-2.5 flex-row">
              {data.authorAvatar ? (
                <Image source={{ uri: data.authorAvatar }} alt="" className="rounded-full object-cover w-4 h-4 bg-slate-200" />
              ) : (
                <View className="w-4 h-4 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-[8px] font-bold text-indigo-600 flex-row">
                  {data.authorName[0]}
                </View>
              )}
              <Text className="text-[10px] font-semibold text-slate-600 dark:text-slate-300 truncate">
                {data.authorName}
              </Text>
            </View>
          )}
