...
// TeacherProfile shows avatars correctly while plain <Image> fails.
function AvatarImage({
  ...
  const fallback = (
    <View className="relative h-9 w-9 sm:h-11 sm:w-11 rounded-full bg-slate-100 dark:bg-slate-800 border-2 border-white dark:border-slate-900 flex items-center justify-center text-xs sm:text-sm font-bold text-slate-600 dark:text-slate-400 shadow-sm transition-all flex-row">
      {initials}
    </View>
  );
...
export default function QuestionCard({ q }: { q: Question }) {
  ...
  if (isDeleting) {
    return (
      <View className="w-full max-w-2xl h-48 flex items-center justify-center bg-slate-50/50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 animate-pulse flex-row">
        <Text className="text-slate-400 dark:text-slate-500 font-medium">Deleting...</Text>
      </View>
    );
  }
  ...
  return (
    <>
    <View className="group relative w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-white/40 dark:border-slate-800/40 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 dark:hover:shadow-blue-500/10 transition-all duration-300 overflow-visible ring-1 ring-slate-200/50 dark:ring-slate-800/50 hover:ring-blue-500/30 dark:hover:ring-blue-500/30">

      {/* Decorative gradient blur */}
      <View className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-500/10 to-fuchsia-500/10 dark:from-blue-500/20 dark:to-fuchsia-500/20 blur-2xl rounded-full -mr-10 -mt-10 pointer-events-none" />

      {/* CARD HEADER: Author & Bounty */}
      <View className="relative flex items-center justify-between p-3.5 sm:p-4 pb-0 flex-row">
        <View className="flex items-center gap-2 sm:gap-3 flex-row">
          {/* Avatar */}
          <Link href={teacherProfileLink} className="shrink-0 relative">
            <View className="absolute inset-0 bg-gradient-to-tr from-blue-500 to-fuchsia-500 rounded-full blur-[2px] opacity-70 group-hover:opacity-100 transition-opacity" />
          ...
          <View className="min-w-0">
            <View className="flex items-center gap-1.5 flex-row">
              <Link href={teacherProfileLink} className="hover:underline decoration-blue-500/30 underline-offset-2 truncate">
                <Text className="font-bold text-slate-900 dark:text-slate-100 text-xs sm:text-sm tracking-tight truncate">{q.createdByName || "Teacher"}</Text>
              </Link>
              <TeacherBadge />
            </View>
            <View className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5 flex-row">
              <Text className="font-medium whitespace-nowrap">{q.subject || 'General'}</Text>
              {q.chapter && (
                <>
                  <Text className="text-slate-300 dark:text-slate-600">·</Text>
                  <Text className="whitespace-nowrap text-indigo-500 dark:text-indigo-400 font-bold">{q.chapter}</Text>
                </>
              )}
              {q.classGrade && (
                <>
                  <Text className="text-slate-300 dark:text-slate-600">·</Text>
                  <Text className="whitespace-nowrap">Class {q.classGrade}</Text>
                </>
              )}
              {formattedDate && (
                <>
                  <Text className="hidden sm:inline">·</Text>
                  <Text className="hidden sm:inline whitespace-nowrap">{formattedDate}</Text>
                </>
              )}
            </View>
          </View>
        ...
        {isOwner && (
          <View className="relative" ref={menuRef}>
            ...
            {menuOpen && (
              <View className="absolute right-0 top-8 w-40 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden z-20 animate-in fade-in zoom-in-95 duration-100">
                <View
                  onPress={handleDelete}
                  className="w-full flex items-center gap-2 px-4 py-3 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-left flex-row"
                >
      ...
      <View className="p-4 sm:p-5 space-y-3 sm:space-y-4">

        {/* Title & Tags */}
        <View className="space-y-1.5 sm:space-y-2">
          <View className="flex flex-wrap items-center gap-1.5 sm:gap-2 flex-row">
            {/* Match The Following Badge */}
            {q.questionType === 'match' && (
              <Text className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-md bg-amber-100/80 dark:bg-amber-900/40 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider flex-row">
            ...
            {(q.points ?? 0) > 15 && q.questionType !== 'match' && (
              <Text className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-md bg-violet-100/80 dark:bg-violet-900/40 border border-violet-200 dark:border-violet-800 text-violet-700 dark:text-violet-300 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider flex-row">
                <FileImage className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                Written
              </Text>
            )}
            {q.difficulty && (
              <Text className={`px-1.5 sm:px-2 py-0.5 rounded-md border text-[9px] sm:text-[10px] font-bold uppercase tracking-wider ${difficultyColor(q.difficulty)}`}>
                {q.difficulty}
              </Text>
            )}
            {q.timeLimit && (
              <Text className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-md bg-slate-100/80 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider flex-row">
                <Clock className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                {q.timeLimit}m
              </Text>
            )}
            {(q.totalAttempts !== undefined && q.totalAttempts > 0) && (
              <Text className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-md bg-blue-50/80 dark:bg-blue-900/40 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider flex-row" title={`${q.solvedCount} students solved this successfully`}>
                <Users className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                <Text className="hidden sm:inline">{q.solvedCount} Solved</Text>
                <Text className="sm:hidden">{q.solvedCount}</Text>
              </Text>
            )}
          </View>

          <Text className="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-100 leading-snug group-hover:text-blue-700 dark:group-hover:text-blue-400 transition-colors">
            {q.title}
          </Text>
        </View>
        ...
        {/* Question Text */}
        {q.body && (
          <Text className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed line-clamp-3">
            {q.body}
          </Text>
        )}
        ...
        {/* Image Attachment */}
        {publicUrl && (
          <View className="group/image relative w-full rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
            <View className="absolute inset-x-0 top-0 h-6 bg-gradient-to-b from-black/20 to-transparent opacity-0 group-hover/image:opacity-100 transition-opacity" />
            <View className="flex items-center justify-center bg-slate-100/50 dark:bg-slate-900/50 p-2 flex-row">
            ...
            <View className="absolute bottom-2 right-2 px-2 py-1 bg-black/60 backdrop-blur-md rounded text-[10px] font-bold text-white uppercase tracking-wider opacity-0 group-hover/image:opacity-100 transition-opacity">
              Attachment
            </View>
          </View>
        )}
        ...
        {q.questionType === 'match' && q.matchPairs && q.matchPairs.length > 0 && (
          <View className="mt-3 bg-slate-50/50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800/80 p-3 sm:p-4">
            <View className="flex items-stretch justify-between gap-4 relative flex-row">
              {/* Left Column Preview */}
              <View className="flex-1 space-y-2 relative z-10 flex-row">
                {q.matchPairs.slice(0, 3).map((pair, idx) => (
                  <View key={`left-${idx}`} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-center shadow-sm relative">
                    <Text className="text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 line-clamp-1">{pair.left}</Text>
                    <View className="absolute top-1/2 -right-2 w-1.5 h-1.5 rounded-full bg-indigo-400 -translate-y-1/2" />
                  </View>
                ))}
              </View>

              {/* Center Decorative Dashed Lines */}
              <View className="w-12 sm:w-16 flex flex-col justify-around items-center opacity-30 dark:opacity-20">
              ...
              <View className="flex-1 space-y-2 relative z-10 flex-row">
                ...
                {q.matchPairs.slice(0, 3).reverse().map((pair, idx) => (
                  <View key={`right-${idx}`} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-center shadow-sm relative">
                    <View className="absolute top-1/2 -left-2 w-1.5 h-1.5 rounded-full bg-indigo-400 -translate-y-1/2" />
                    <Text className="text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 line-clamp-1">{pair.right}</Text>
                  </View>
                ))}
              </View>
            </View>
            {q.matchPairs.length > 3 && (
              <View className="mt-2 text-center text-[10px] sm:text-xs text-slate-400 dark:text-slate-500 font-medium">
      ...
      <View className="px-4 sm:px-5 pb-4 sm:pb-5 pt-1 sm:pt-2 flex items-center justify-between gap-3 sm:gap-4 flex-row">
        <View className="flex items-center gap-2 sm:gap-3 flex-row">
          {/* Bounty Badge */}
          <View className="flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 text-amber-900 dark:text-amber-300 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg border border-amber-100/80 dark:border-amber-900/50 shadow-sm shrink-0 flex-row">
            <View className="bg-amber-100 dark:bg-amber-900/50 p-1 rounded-full">
              <Zap className="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-amber-500 text-amber-500 dark:text-amber-400" />
            </View>
            <Text className="text-[10px] sm:text-xs font-bold">{q.points ?? 0} <Text className="opacity-70 font-normal ml-0.5 hidden sm:inline">Points</Text></Text>
          </View>
          ...
          >
            ...
            <Text className="hidden sm:inline">{isSaved ? "Saved" : "Save"}</Text>
          </View>
          ...
          {user && !isTeacher && Array.isArray(q.options) && q.options.length > 0 && !q.hasAttempted && (
            <View
              ...
              className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/40 transition-colors text-[10px] sm:text-xs font-bold uppercase tracking-wider flex-row"
              aria-label="Challenge a friend"
            >
              <Swords className="w-3.5 h-3.5" />
              <Text className="hidden sm:inline">Duel</Text>
            </View>
          )}
        </View>
        ...
        {user === undefined ? (
          <View className="flex-1 sm:flex-none flex-row">
            <View className="w-full h-8 sm:h-10 bg-slate-200 dark:bg-slate-700 animate-pulse rounded-lg sm:rounded-xl"></View>
          </View>
        ) : isTeacher ? (
          <View className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 sm:gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 px-3 py-2 sm:px-5 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold cursor-default select-none transition-all flex-row">
            <TeacherBadge />
            <Text className="hidden sm:inline">Teacher View</Text>
            <Text className="sm:hidden">Teacher</Text>
          </View>
        ) : q.hasWrittenSubmission && q.userSubmissionId ? (
          <Link href={`/questions/${q.id}`} className="flex-1 sm:flex-none flex-row">
            <View className="w-full flex items-center justify-center gap-1.5 sm:gap-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white px-3 py-2 sm:px-5 sm:py-2.5 rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm transition-all shadow-md shadow-emerald-500/20 hover:shadow-lg hover:shadow-emerald-500/30 hover:-translate-y-0.5 active:translate-y-0 flex-row">
              <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
              <Text className="hidden sm:inline">See Breakdown</Text>
              <Text className="sm:hidden">Details</Text>
            </View>
          </Link>
        ) : q.hasAttempted ? (
          <Link href={`/questions/${q.id}`} className="flex-1 sm:flex-none flex-row">
            <View className="w-full flex items-center justify-center gap-1.5 sm:gap-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white px-3 py-2 sm:px-5 sm:py-2.5 rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm transition-all shadow-md shadow-emerald-500/20 hover:shadow-lg hover:shadow-emerald-500/30 hover:-translate-y-0.5 active:translate-y-0 flex-row">
              ...
              <Text className="hidden sm:inline">Solved Details</Text>
              <Text className="sm:hidden">Solved</Text>
            </View>
          </Link>
        ) : (
          <Link href={`/questions/${q.id}`} className="flex-1 sm:flex-none flex-row">
