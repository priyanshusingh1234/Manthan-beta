...
const IncomingCallKit = { requestPermissions: () => {}, requestFullScreenIntentPermission: () => {}, showIncomingCall: () => {} };
      ...
      ? createPortal(
          <>
            {incomingCall && (
              ...
              >
                <View className="flex-1 flex flex-col items-center justify-center text-center gap-6">
                  <View className="relative">
                    ...
                    />
                    <View className="relative h-28 w-28 rounded-full overflow-hidden border-4 border-white/20 shadow-2xl">
                      {incomingCall.callerAvatar ? (
                        <Image source={{ uri: incomingCall.callerAvatar }} alt={incomingCall.callerName} fill className="object-cover" />
                      ) : (
                        <View className="w-full h-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-4xl font-black flex-row">
                  ...
                  <View>
                    <Text className="text-white/60 text-sm font-semibold tracking-widest uppercase mb-2">
                      Incoming {incomingCall.type === 'video' ? 'Video' : 'Voice'} Call
                    </Text>
                    <Text className="text-3xl font-black tracking-tight">{incomingCall.callerName}</Text>
                  </View>
                ...
                <View className="flex items-center justify-center gap-16 w-full flex-row">
                  <View className="flex flex-col items-center gap-3">
                    <View
                      onPress={declineCall}
                      className="flex items-center justify-center h-[72px] w-[72px] rounded-full bg-red-500 shadow-xl shadow-red-500/30 active:scale-95 transition-transform flex-row"
                    >
                      <PhoneOff className="h-8 w-8" />
                    </View>
                    <Text className="text-xs font-bold text-white/50 uppercase tracking-widest">Decline</Text>
                  </View>

                  <View className="flex flex-col items-center gap-3">
                    <View
                      onPress={acceptCall}
                      className="flex items-center justify-center h-[72px] w-[72px] rounded-full bg-green-500 shadow-xl shadow-green-500/30 active:scale-95 transition-transform flex-row"
                    >
                      {incomingCall.type === 'video' ? <Video className="h-8 w-8" /> : <Phone className="h-8 w-8" />}
                    </View>
                    <Text className="text-xs font-bold text-white/50 uppercase tracking-widest">Accept</Text>
                  </View>
                </View>
              </View>
            )}
