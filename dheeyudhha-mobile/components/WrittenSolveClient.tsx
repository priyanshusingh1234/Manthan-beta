...
import { CheckCircle2, Loader2, Clock, Zap, ArrowLeft, Shield, Users, Send } from 'lucide-react-native';
...
export default function WrittenSolveClient({ question }: { question: WrittenQuestion }) {
    ...
    if (!authChecked) {
        return (
            <View className="flex flex-col items-center justify-center p-20 text-slate-400 dark:text-slate-500 space-y-4">
                ...
                <Text className="font-medium animate-pulse">Loading written challenge...</Text>
            </View>
        );
    }
    ...
    return (
        <View className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-5 pb-16">
            {/* Co-op banner */}
            {challengeId && (
                <View className="bg-gradient-to-r from-indigo-500 to-purple-500 rounded-2xl p-4 text-white shadow-md flex items-center justify-center gap-3 animate-pulse flex-row">
                    <Users className="w-6 h-6" />
                    <Text className="font-bold">Co-op Challenge Mode Active! You&apos;re playing to win points together.</Text>
                </View>
            )}
            ...
            <View className="flex items-center justify-between bg-white/95 dark:bg-slate-900/90 backdrop-blur-md px-5 py-3 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm sticky top-4 z-40 flex-row">
                <View onPress={() => router.back()} className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors text-sm font-medium flex-row">
                    <ArrowLeft className="w-4 h-4" /> Back
                </View>
                <View className="flex items-center gap-3 flex-row">
                    {!activeSubmission && (
                        <View className={`flex items-center gap-2 font-mono text-sm font-bold px-4 py-1.5 rounded-full border transition-colors ${isTimeLow ? "bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800/50 animate-pulse" : "bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700"}`}>
                            <Clock className="w-4 h-4" />
                            {formatSolveTime(solveTimeLeft)}
                        </View>
                    )}
                    <View className="flex items-center gap-2 bg-violet-600 text-white px-4 py-1.5 rounded-full text-sm font-bold shadow-sm flex-row">
            ...
            {activeSubmission && (
                <>
                    {/* Status badge */}
                    <View className={`flex items-center gap-2 px-4 py-3 rounded-2xl border text-sm font-semibold ${statusInfo.color}`}>
                        {statusInfo.icon}
                        {statusInfo.label}
                    </View>
