...
import { Zap, Star, Trophy, ChevronRight } from 'lucide-react-native';
...
export default function LevelUpModal({ isOpen, newLevel, onClose }: LevelUpModalProps) {
  ...
  return (
    <>
      ...
      {/* ── Backdrop ── */}
      <View
        className="fixed inset-0 z-[999] flex items-center justify-center p-4 flex-row"
        style={{
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Spinning star ring */}
            <View
              className="absolute inset-0 flex items-center justify-center pointer-events-none flex-row"
            ...
            >
              ...
              <Text className="text-white font-black text-3xl leading-none drop-shadow-lg">{newLevel}</Text>
            </View>
          </View>
          ...
          <View className="space-y-2 mb-8">
            ...
            <Text className="text-white/50 text-sm font-bold mt-1">
          ...
          {/* ── Stat pill ── */}
          <View
            className="flex items-center gap-3 px-5 py-2.5 rounded-2xl border mb-8 w-full justify-center flex-row"
            style={{ background: `${palette.from}14`, borderColor: `${palette.from}40` }}
          >
            <Trophy className="w-4 h-4" style={{ color: palette.from }} />
            <Text className="text-sm font-black text-white/80">
          ...
          <View
            onPress={onClose}
            className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl text-white font-black text-base relative overflow-hidden group flex-row"
            style={{
              background: gradientCss,
          ...
          >
            ...
            />
            <Text className="relative z-10">Keep Grinding 🔥</Text>
            <ChevronRight className="w-5 h-5 relative z-10" />
          </View>

          <Text className="text-white/25 text-xs mt-4 font-medium">Tap anywhere to dismiss</Text>
        </View>
      </View>
    </>
  );
