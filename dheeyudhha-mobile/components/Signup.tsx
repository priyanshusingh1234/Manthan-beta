...
import { Mail, Lock, Eye, EyeOff, Chrome, User, GraduationCap, ArrowRight, Loader2 } from 'lucide-react-native';
...
const Signup: React.FC = () => {
  ...
  return (
    ...
    */
    <View className="min-h-screen bg-slate-50 dark:bg-slate-950">

      {/* Top nav bar — sticky so it doesn't scroll away */}
      <View className="sticky top-0 z-30 flex items-center px-4 py-3 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur border-b border-slate-200 dark:border-slate-800 flex-row">
        <Link href="/" className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors flex-row">
          <Svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <Path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </Svg>
          <Text className="text-xs font-black uppercase tracking-widest hidden sm:block">Back</Text>
        </Link>
        <View className="flex items-center gap-2 mx-auto flex-row">
          <View className="w-7 h-7 rounded-xl bg-gradient-to-br from-indigo-500 to-fuchsia-500 flex items-center justify-center shadow-md shadow-indigo-500/30 flex-row">
            <Svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <Path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </Svg>
          </View>
          <Text className="text-sm font-black italic tracking-tighter text-slate-900 dark:text-white">DHEEYUDHA</Text>
        </View>
        <View className="w-16 sm:w-24" /> {/* spacer to center logo */}
      </View>

      {/* Page body — single column, flows naturally */}
      <View className="max-w-md mx-auto px-4 pt-8 pb-20">

        {/* Hero */}
        <View className="text-center mb-8">
          <Text className="inline-block bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full mb-4">
            Join the Battle
          </Text>
          <Text className="text-4xl sm:text-5xl font-black italic tracking-tighter uppercase leading-tight text-slate-900 dark:text-white">
            Enter The<br />
            <Text className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-fuchsia-500">War Room.</Text>
          </Text>
          <Text className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">Create your account and forge your legacy.</Text>
        </View>
        ...
        <View
          ...
          className="w-full flex items-center justify-center gap-3 py-3.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm hover:shadow-md hover:border-indigo-300 active:scale-95 transition-all mb-6 flex-row"
        >
          <Chrome className="w-5 h-5 text-slate-600 dark:text-slate-300" />
          <Text className="text-sm font-black uppercase tracking-widest text-slate-700 dark:text-slate-300">Continue with Google</Text>
        </View>
        ...
        <View className="relative mb-6">
          ...
          <View className="relative flex justify-center flex-row">
            <Text className="bg-slate-50 dark:bg-slate-950 px-4 text-[10px] font-black uppercase tracking-widest text-slate-400">or sign up with email</Text>
          </View>
        </View>

        {/* Form card */}
        <View onPress={handleSubmit}>
          <View className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">

            {/* Username + Full Name */}
            <View className="grid grid-cols-2 gap-3">
              <View>
                <Text className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Username</Text>
              ...
              <View>
                <Text className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Full Name</Text>
                <View className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                  <TextInput
            ...
            {/* Email */}
            <View>
              <Text className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Email</Text>
              <View className="relative">
            ...
            <View className="grid grid-cols-2 gap-3">
              <View>
                <Text className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Password</Text>
                <View className="relative">
                  ...
                  />
                  <View type="button" onPress={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">
                    {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </View>
                </View>
              </View>
              <View>
                <Text className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Confirm</Text>
                <View className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                  <TextInput
            ...
            {/* Class */}
            <View>
              <Text className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Class / Grade</Text>
              <View className="relative">
            ...
            <View className="space-y-3 pt-1">
              <Text className="flex items-start gap-3 cursor-pointer flex-row">
                <View className="relative flex-shrink-0 mt-0.5 flex-row">
                  <TextInput type="checkbox" required checked={ageConfirmed} onChange={(e) => setAgeConfirmed(e.target.checked)}
                ...
                <Text className="text-xs font-medium text-slate-600 dark:text-slate-400 leading-relaxed">I confirm I am at least 14 years old</Text>
              </Text>
              <Text className="flex items-start gap-3 cursor-pointer flex-row">
                <View className="relative flex-shrink-0 mt-0.5 flex-row">
                  <TextInput type="checkbox" required checked={agreeToTerms} onChange={(e) => setAgreeToTerms(e.target.checked)}
                    className="peer appearance-none w-5 h-5 rounded-lg border-2 border-slate-300 dark:border-slate-600 checked:bg-indigo-500 checked:border-indigo-500 transition-all" />
                  <CheckIcon className="absolute inset-0 w-5 h-5 text-white opacity-0 peer-checked:opacity-100 transition-opacity pointer-events-none p-0.5" />
                </View>
                <Text className="text-xs font-medium text-slate-600 dark:text-slate-400 leading-relaxed">
            ...
            {/* Error */}
            {error && (
              <View className="p-3 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30">
                <Text className="text-xs font-black uppercase tracking-wide text-red-600 dark:text-red-400">{error}</Text>
              </View>
            )}
            ...
            <View
              ...
              className="w-full py-4 bg-gradient-to-r from-indigo-600 to-fuchsia-600 text-white font-black text-sm italic uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-indigo-500/30 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50 mt-2 flex-row"
            >
        ...
        <Text className="text-center text-sm font-medium text-slate-500 dark:text-slate-400 mt-6">
