...
const TeacherProfile: React.FC = () => {
  ...
  return (
    <View className="min-h-[100dvh] bg-slate-50 dark:bg-slate-950 relative pb-20">
      ...
      <View className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Crop Modal */}
        {showCrop && imageSrc && (
          <CropModal
            imageSrc={imageSrc}
        ...
        {/* Profile Header */}
        <View className="relative z-20 mt-16 bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] overflow-hidden mb-12 border border-slate-100 dark:border-slate-800">
          <View className="h-44 sm:h-64 relative group shadow-inner">
          ...
          <View className="px-5 sm:px-8 pb-8 relative mt-0 sm:mt-0">
            {/* Top Row: Avatar and Stats - Instagram Style */}
            <View className="flex flex-col sm:flex-row sm:items-start gap-5 sm:gap-10">
              
              {/* Avatar Section */}
              <View className="flex items-center sm:items-start gap-6 sm:gap-8 w-full sm:w-auto -mt-10 sm:-mt-20 flex-row">
                <View className="relative group shrink-0 z-10">
                  <View className="w-24 h-24 sm:w-40 sm:h-40 rounded-full overflow-hidden border-4 sm:border-[6px] border-white dark:border-slate-900 shadow-2xl bg-white dark:bg-slate-800">
                    {typeof userData.avatar === 'string' && userData.avatar.startsWith('http') ? (
                      ...
                      <View className="w-full h-full flex items-center justify-center text-4xl sm:text-6xl flex-row">
                        {userData.avatar}
                      </View>
                    )}
                    {avatarUploading && (
                ...
                {/* Mobile-only Stats */}
                <View className="flex-1 sm:hidden flex-row">
                   {currentUser && <FollowButton profileUserId={currentUser.id} />}
                </View>
              </View>

              {/* User Identity Section */}
              <View className="flex-1 text-left z-10 flex-row">
                {message && (
                  <View className="mb-4 inline-block px-4 py-1.5 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 rounded-lg text-sm font-bold shadow-sm">
                    {message}
                  </View>
                )}

                <View className="flex flex-col gap-1">
                  <View className="flex items-center gap-2 flex-wrap flex-row">
                    <Text className="text-2xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tighter flex items-center gap-2 flex-row">
                       {userData.name}
                       {currentUser?.user_metadata?.isTeacher && <TeacherBadge />}
                    </Text>
                  </View>
                  <View className="flex items-center gap-2 flex-wrap mt-1 flex-row">
                    <Text className="text-slate-500 text-sm font-bold tracking-tight">@{userData.username || 'username'}</Text>
                ...
                <View className="mt-4 flex flex-col gap-1">
                  ...
                  <Text className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed max-w-xl">
                    {userData.bio || 'Inspiring students to reach their full potential 🚀'}
                  </Text>
                </View>

                {/* Actions Row */}
                <View className="mt-6 flex flex-wrap items-center gap-3 flex-row">
                  <View
                    onPress={() => setShowEditProfile(true)}
                    className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-bold rounded-xl transition-all active:scale-95 text-sm flex-row"
                  >
                    Edit Profile
                  </View>
                  <Link
                    href="/league"
                    className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 font-bold rounded-xl transition-all active:scale-95 text-sm border border-indigo-200 dark:border-indigo-700 flex-row"
                  >
        ...
        {/* Stats Grid */}
        <View className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 relative z-10">
          {stats.map((stat, index) => (
            ...
            >
              <View className="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-gradient-to-br from-slate-100 to-transparent dark:from-slate-800 rounded-full blur-2xl opacity-50 group-hover:opacity-100 transition-opacity"></View>

              <View className={`${stat.bgColor} w-14 h-14 rounded-2xl flex items-center justify-center mb-5 border border-white/20 shadow-sm relative z-10 group-hover:scale-110 transition-transform`}>
                <stat.icon className={`w-7 h-7 ${stat.color} drop-shadow-sm`} />
              </View>
              <View className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-1 relative z-10 drop-shadow-sm">{stat.value}</View>
              <View className="text-sm font-bold text-slate-500 dark:text-slate-400 tracking-wide uppercase relative z-10">{stat.label}</View>
            </View>
          ))}
        </View>
        ...
        <View className="grid grid-cols-1 lg:grid-cols-2 gap-8 relative z-10">
          ...
          {/* Recent Questions */}
          <View className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm p-8 border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
            <View className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-400 via-blue-500 to-cyan-500 opacity-50 group-hover:opacity-100 transition-opacity"></View>
            <View className="flex items-center gap-4 mb-8 flex-row">
              ...
              <Text className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Recent Questions</Text>
            </View>
            <View className="space-y-4">
              {recentBattles.map((battle, index) => (
                ...
                >
                  <View className="flex items-center gap-4 mb-4 flex-row">
                    <View className="p-3 bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-indigo-900/40 dark:to-blue-900/40 rounded-xl shadow-inner group-hover/battle:scale-105 transition-transform border border-indigo-100 dark:border-indigo-800">
                      <Sword className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                    </View>
                    <View>
                      <Text className="font-bold text-lg text-slate-900 dark:text-white group-hover/battle:text-indigo-600 transition-colors">{battle.title}</Text>
                      <Text className="text-sm font-medium text-slate-500 dark:text-slate-400">{battle.points} Points • {battle.views} Views</Text>
                    </View>
                  </View>
                  <View className="flex items-center justify-end text-sm font-bold text-slate-400 dark:text-slate-500 pt-3 border-t border-slate-50 dark:border-slate-800/50 flex-row">
                    <Text>{battle.date}</Text>
                  </View>
                </View>
              ))}
            </View>
            <View className="w-full mt-8 py-4 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-black rounded-2xl transition-all duration-300 active:scale-95 border border-slate-200 dark:border-slate-700 shadow-sm">
              View All Questions
            </View>
          </View>
        </View>
