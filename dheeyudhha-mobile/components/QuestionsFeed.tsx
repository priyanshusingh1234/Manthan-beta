...
import { RefreshCw, X, ChevronDown, ArrowUp, Loader2 } from 'lucide-react-native';
...
function ShimmerCard() {
  return (
    <View className="bg-white dark:bg-slate-900 rounded-3xl p-5 shadow-sm border border-slate-100 dark:border-slate-800/60 overflow-hidden relative">
      <View className="absolute inset-0 -translate-x-full animate-[shimmer_1.4s_infinite] bg-gradient-to-r from-transparent via-white/20 dark:via-slate-700/30 to-transparent" />
      <View className="h-2.5 w-20 bg-slate-200 dark:bg-slate-800 rounded-full mb-4" />
      <View className="flex items-center gap-3 mb-4 flex-row">
        <View className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-800 shrink-0" />
        <View className="flex-1 space-y-2 flex-row">
      ...
      <View className="space-y-2.5 mb-5">
        <View className="h-4 w-3/4 bg-slate-200 dark:bg-slate-800 rounded-full" />
        <View className="h-4 w-full bg-slate-200 dark:bg-slate-800 rounded-full" />
        <View className="h-4 w-2/3 bg-slate-200 dark:bg-slate-800 rounded-full" />
      </View>
      <View className="h-10 w-full bg-slate-100 dark:bg-slate-800/50 rounded-xl" />
    </View>
  );
}
    ...
    <>
      ...
      <View className="space-y-3">
        ...
        <View className="space-y-2">
          {/* Subject pills */}
          <View className="flex gap-1.5 overflow-x-auto scrollbar-none pb-0.5 flex-row">
            {SUBJECTS.map(s => {
              const active = subject === s.value;
              return (
                <View
                  key={s.value}
                  onPress={() => setSubject(active ? '' : s.value)}
          ...
          <View className="flex items-center justify-between gap-2 flex-row">
            ...
            <View className="flex items-center gap-2 ml-auto flex-row">
              {hasFilter && (
                <>
                  <View
                    onPress={() => { window.dispatchEvent(new Event('open_daily_planner')); }}
                    className="flex items-center gap-1 text-[11px] font-black text-indigo-500 active:scale-95 transition-transform mr-1 flex-row"
                  >
                    Change Plan
                  </View>
                  <View
                    onPress={() => { setSubject(''); setClass(''); setChapter(''); }}
                    className="flex items-center gap-1 text-[11px] font-black text-red-500 active:scale-95 transition-transform flex-row"
                  >
              ...
              <View
                ...
                disabled={refreshing}
                className="flex items-center gap-1.5 text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-100 dark:border-indigo-900/60 px-3 py-1.5 rounded-full transition-all active:scale-95 disabled:opacity-60 flex-row"
              >
        ...
        {showNewBanner && (
          <View
            ...
            className="sticky top-3 z-30 w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-indigo-600 text-white text-xs font-black rounded-2xl shadow-lg shadow-indigo-500/30 animate-in slide-in-from-top duration-300 active:scale-95 transition-transform flex-row"
          >
        ...
        {loading ? skeleton : err ? (
          <View className="py-6 text-sm text-red-500">Error: {err}</View>
        ) : filtered.length === 0 ? (
          <View className="py-10 text-center space-y-2">
            <View className="text-4xl">🔍</View>
            <Text className="text-sm font-bold text-slate-700 dark:text-slate-300">No questions found</Text>
            <Text className="text-xs text-slate-400">
              {hasFilter ? 'Try a different subject or class.' : 'Nothing in your feed yet.'}
            </Text>
            {hasFilter && (
              <View onPress={() => { setSubject(''); setClass(''); }} className="mt-2 px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-black active:scale-95">
                Clear filters
              </View>
            )}
          </View>
          ...
          <>
              ...
              <React.Fragment key={item.id}>
                <View className="space-y-1">
                  {item._feedLabel && item.type !== 'post' && (
                    <Text className="text-xs font-semibold text-indigo-500 dark:text-indigo-400 px-1 tracking-wide">
                      {item._feedLabel}
                    </Text>
                  )}
                  {item.type === 'post' ? (
            ...
            <View ref={sentinelRef} className="py-3 flex justify-center flex-row">
              {loadingMore ? (
                <View className="flex items-center gap-2 text-xs text-slate-400 flex-row">
                  <Loader2 className="w-3.5 h-3.5 animate-spin" /> Loading more…
                </View>
              ) : exhausted || filterModeRef.current ? (
                visibleCount >= filtered.length && (
                  <Text className="text-xs text-slate-400">All {filtered.length} questions shown ✓</Text>
                )
              ) : null}
            </View>
          </>
