...
import { Home, Compass, LucideIcon, PlaySquare, MessageSquare, MessageCircle, Settings, Flame } from 'lucide-react-native';
...
const BottomNav: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <View className="absolute inset-0 bg-gradient-to-b from-white/40 dark:from-slate-800/20 to-white/10 dark:to-transparent pointer-events-none" />

      <View className="relative flex items-center justify-between px-2 h-16 max-w-lg mx-auto flex-row">
        {navItems.map((item, index) => {
          ...
          if (item.isCenter) {
            return (
              <View key={item.href} className="relative -top-8 group">
                <Link
                  href={item.href}
                ...
                >
                  ...
                  <View className="absolute inset-0 rounded-full border border-white/20 animate-pulse active:hidden" />
                </Link>
                <View className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-purple-900/50 dark:text-purple-400/70 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                  Clips
                </View>
              </View>
            );
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <View className="absolute top-2 w-10 h-8 bg-blue-50 dark:bg-blue-900/40 rounded-xl -z-10 transition-all duration-300 transform scale-100 opacity-100 border border-blue-100/50 dark:border-blue-800/50" />
              )}
              ...
              {/* Active Dot */}
              {isActive && (
                <View className="absolute bottom-1.5 w-1 h-1 bg-blue-600 dark:bg-blue-400 rounded-full shadow-[0_0_8px_currentColor]" />
              )}
            </Link>
          );
        })}
