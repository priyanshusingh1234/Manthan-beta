...
import { Upload, Loader2, FileImage, AlertTriangle, X, Camera } from 'lucide-react-native';
...
export default function UploadSection({
    ...
    useEffect(() => {
        ...
        setUseCapacitorAndroidCamera((Platform.OS !== 'web') && Platform.OS === "android");
    }, []);
    ...
    return (
        <View className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 sm:p-8 shadow-sm border border-gray-100 dark:border-slate-800">
            ...
            {!previewUrl ? (
                <View className="w-full space-y-3">
                    {isMobile && (hasCameraSupport || useCapacitorAndroidCamera) ? (
                        ...
                        >
                            <View className="w-14 h-14 bg-violet-100 dark:bg-violet-900/40 rounded-2xl flex items-center justify-center pointer-events-none flex-row">
                                {cameraLoading ? (
                            ...
                            <Text className="text-slate-600 dark:text-slate-300 font-medium pointer-events-none text-center px-4">
                                {cameraLoading ? "Opening camera..." : useCapacitorAndroidCamera ? "Open native camera and take photo" : "Open camera and take photo"}
                            </Text>
                            <Text className="text-slate-400 dark:text-slate-500 text-xs pointer-events-none uppercase font-bold tracking-widest">
                                Camera first mode
                            </Text>
                        </View>
                    ) : (
                    ...
                    {isMobile && (
                        <Text id="answer-upload-fallback" className="block text-center text-xs font-semibold text-slate-500 dark:text-slate-400 cursor-pointer hover:text-violet-600 dark:hover:text-violet-400">
                            Use gallery instead
                            <TextInput
                    ...
                    {cameraError && (
                        <Text className="text-xs text-red-600 dark:text-red-400 text-center font-medium">{cameraError}</Text>
                    )}

                    {showCamera && stream && (
                        <View className="rounded-2xl border border-violet-200 dark:border-violet-800/50 bg-violet-50 dark:bg-violet-900/20 p-3 space-y-3">
                            <video
                            ...
                            <View className="flex gap-2 flex-row">
                                <View
                                    type="button"
                                    onPress={captureFromStream}
                                    disabled={isCapturing}
                                    className="flex-1 flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:bg-slate-300 dark:disabled:bg-slate-700 text-white font-bold py-2.5 rounded-xl transition-all flex-row"
                                >
                ...
                <View className="w-full rounded-2xl border-2 border-violet-300 dark:border-violet-700 bg-violet-50 dark:bg-violet-900/20 overflow-hidden">
                    ...
                    {/* File info bar */}
                    <View className="px-4 py-2.5 flex items-center justify-between bg-violet-50 dark:bg-violet-900/20 border-t border-violet-200 dark:border-violet-800/50 flex-row">
                        <View className="flex items-center gap-2 text-xs text-violet-700 dark:text-violet-300 font-medium min-w-0 flex-row">
                            <FileImage className="w-3.5 h-3.5 shrink-0" />
                            <Text className="truncate">{selectedFile?.name || "Captured Answer"}</Text>
                        </View>
                        <Text id="answer-upload-change" className="ml-3 text-xs text-violet-600 dark:text-violet-400 font-bold cursor-pointer hover:text-violet-800 dark:hover:text-violet-300 shrink-0">
                            Change
                            <TextInput
            ...
            {/* Time's up warning */}
            {solveTimeLeft === 0 && (
                <View className="mt-3 px-4 py-2.5 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800/50 rounded-xl text-sm text-red-700 dark:text-red-400 font-medium flex items-center gap-2 flex-row">
