    ...
    <>
      {promo && (
        <View
          ...
          className="fixed inset-0 z-[9990] flex items-end justify-center p-4 pb-10 flex-row"
        ...
        >
          <>
            {showContent && (
              ...
              >
                {/* Top glow line */}
                <View className="h-[3px] w-full" style={{ background: `linear-gradient(90deg, transparent, ${c}, transparent)` }} />
                ...
                <View className="p-7 flex flex-col items-center text-center relative">
                  {/* Pill label */}
                  <View initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                  ...
                  {promo.isWelcome ? (
                    ...
                    <View initial={{ opacity: 0, scale: 0.5, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
                    ...
                    /* Promotion: from → to */
                    <View className="flex items-center gap-5 mb-6 flex-row">
                      <View initial={{ opacity: 0, x: -24 }} animate={{ opacity: 0.45, x: 0 }} transition={{ delay: 0.2 }}
                        className="flex flex-col items-center gap-1.5">
                        <LeagueBadge name={promo.from!} size={58} />
                        <Text className="text-[10px] text-white/35 font-bold">{promo.from}</Text>
                      </View>
                      <Text initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }}
                        transition={{ delay: 0.32, type: 'spring' }} className="text-2xl text-white/60">→</Text>
                      <View initial={{ opacity: 0, x: 24, scale: 0.65 }} animate={{ opacity: 1, x: 0, scale: 1.08 }}
                        ...
                        <Text className="text-xs font-black" style={{ color: c }}>{promo.to}</Text>
                      </View>
                    </View>
                  )}

                  {/* Text */}
                  <View initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                    {promo.isWelcome ? (
                      <>
                        <Text className="text-2xl font-black text-white leading-tight">You're in</Text>
                        <Text className="text-3xl font-black leading-tight" style={{ color: c }}>{promo.to} League</Text>
                        <Text className="text-white/45 text-sm font-medium mt-2.5 leading-relaxed">
                          Earn points every month to climb leagues.<br />Rankings reset on the 1st of each month.
                        </Text>
                      </>
                    ) : (
                      <>
                        <Text className="text-2xl font-black text-white leading-tight">Welcome to</Text>
                        <Text className="text-3xl font-black leading-tight" style={{ color: c }}>{promo.to} League!</Text>
                        <Text className="text-white/45 text-sm font-medium mt-2.5 leading-relaxed">
                  ...
                  {/* Buttons */}
                  <View initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.72 }}
                    className="mt-7 w-full space-y-2.5">
                    <View onPress={() => { dismiss(); router.push('/league'); }}
