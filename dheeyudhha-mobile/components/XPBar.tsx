...
export default function XPBar({ xp = 0, compact = false }: XPBarProps) {
    ...
    if (compact) {
        return (
            <View className="flex items-center gap-2 w-full flex-row">
                {/* Level badge */}
                <View
                    className="flex items-center gap-1 px-2 py-0.5 rounded-full text-white shrink-0 flex-row"
                    style={{ background: `linear-gradient(135deg, ${color.from}, ${color.to})` }}
                >
                    <Zap className="w-2.5 h-2.5" />
                    <Text className="text-[10px] font-black">Lv.{level}</Text>
                </View>

                {/* Progress bar */}
                <View className="flex-1 h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden flex-row">
                    <View
        ...
        <View className="w-full">
            ...
            <View className="flex items-center justify-between mb-2 flex-row">
                <View className="flex items-center gap-2 flex-row">
                    <View
                        className="w-9 h-9 rounded-xl flex items-center justify-center shadow-lg flex-row"
                        style={{ background: `linear-gradient(135deg, ${color.from}, ${color.to})` }}
                    >
                        <Zap className="w-4.5 h-4.5 text-white" />
                    </View>
                    <View>
                        <Text className="font-black text-sm text-slate-900 dark:text-white leading-none">
                            Level {level}
                            <Text
            ...
            <View className="w-full h-3 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden border border-slate-200 dark:border-slate-700">
                ...
                >
                    {/* Shine effect */}
                    <View className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent rounded-full" />
                </View>
            </View>

            {/* Milestone ticks */}
            <View className="flex justify-between mt-1 px-0.5 flex-row">
                {[0, 25, 50].map(tick => (
