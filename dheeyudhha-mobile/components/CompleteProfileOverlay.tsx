...
import { User as UserIcon, GraduationCap, Loader2, Sparkles, Sword, Building2, Trophy, PlaySquare, Shield } from 'lucide-react-native';
import { NativeAudio } from '@capacitor-community/native-audio';
const StatusBar = { setOverlaysWebView: () => {}, setStyle: () => {}, setBackgroundColor: () => {} };
...
export default function CompleteProfileOverlay({ onComplete }: { onComplete?: () => void }) {
  ...
  if (step === null) {
    return (
      <View className="fixed inset-0 z-[9999] bg-slate-50 dark:bg-slate-950 flex items-center justify-center flex-row">
  ...
  return (
    <View className="fixed inset-0 z-[9999] bg-slate-50 dark:bg-slate-950 overflow-hidden flex flex-col pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
      {/* Dynamic Background */}
      <View className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 via-purple-500/10 to-pink-500/10 dark:from-indigo-900/20 dark:via-purple-900/20 dark:to-pink-900/20 pointer-events-none" />
      ...
      <>
        {step === 1 && (
          ...
          >
            <View className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-fuchsia-500 rounded-3xl mx-auto flex items-center justify-center mb-6 shadow-xl shadow-indigo-500/30 shrink-0 flex-row">
            ...
            <Text className="text-center text-slate-500 dark:text-slate-400 text-sm font-medium mb-8">
              {isPreFilled ? "Welcome! Here are the details you provided:" : "Welcome to the Arena. Forging your identity..."}
            </Text>

            <View onPress={handleProfileSubmit} className="space-y-4 w-full">
              ...
              {error && (
                <View initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-xs font-bold text-red-500 text-center p-3 bg-red-50 dark:bg-red-500/10 rounded-xl">
                  {error}
                </View>
              )}
              ...
              <View
                type="submit"
                disabled={loading}
                className="w-full py-4 mt-6 bg-gradient-to-r from-indigo-600 to-fuchsia-600 text-white font-black text-sm uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-indigo-500/30 active:scale-95 transition-transform flex justify-center items-center gap-2 flex-row"
              >
        ...
        {step === 2 && (
          ...
          >
            ...
            <Text className="text-3xl font-black italic uppercase tracking-tighter text-center text-slate-900 dark:text-white mb-6">
              Welcome to the Arena
            </Text>
            <Text className="text-center text-slate-500 dark:text-slate-400 text-base font-medium mb-8 leading-relaxed">
            ...
            <View className="space-y-4 w-full mb-8">
              <View className="flex items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl border-2 border-slate-100 dark:border-slate-700 shadow-sm flex-row">
                <View className="w-12 h-12 bg-amber-100 dark:bg-amber-900/30 rounded-xl flex items-center justify-center shrink-0 flex-row">
                  <Trophy className="w-6 h-6 text-amber-500" />
                </View>
                <View>
                  <Text className="font-bold text-slate-800 dark:text-slate-200 text-sm">Leaderboards</Text>
                  <Text className="text-xs text-slate-500">Answer questions to climb the global ranks.</Text>
                </View>
              </View>
              
              <View className="flex items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl border-2 border-slate-100 dark:border-slate-700 shadow-sm flex-row">
                <View className="w-12 h-12 bg-purple-100 dark:bg-purple-900/30 rounded-xl flex items-center justify-center shrink-0 flex-row">
                  <PlaySquare className="w-6 h-6 text-purple-500" />
                </View>
                <View>
                  <Text className="font-bold text-slate-800 dark:text-slate-200 text-sm">Video Clips</Text>
                  <Text className="text-xs text-slate-500">Learn from bite-sized educational reels.</Text>
                </View>
              </View>
            </View>
        ...
        {step === 3 && (
          <View 
            ...
            className="flex-1 flex flex-col items-center justify-center p-6 relative z-10 w-full max-w-md mx-auto"
          >
            <View className="w-20 h-20 bg-gradient-to-br from-rose-500 to-orange-500 rounded-full mx-auto flex items-center justify-center mb-8 shadow-2xl shadow-rose-500/40 shrink-0 flex-row">
            ...
            <Text className="text-center text-slate-500 dark:text-slate-400 text-base font-medium mb-8 leading-relaxed">
              The ultimate test of knowledge is the <Text>1v1 Duel</Text>. Challenge your classmates, answer faster, and steal their points.
            </Text>

            <View className="w-full bg-rose-50 dark:bg-rose-900/10 border-2 border-rose-200 dark:border-rose-900/50 rounded-2xl p-6 mb-8 text-center">
              <Text className="text-rose-800 dark:text-rose-300 font-bold mb-2">Rules of the Duel:</Text>
              <View className="text-sm text-rose-600 dark:text-rose-400 space-y-2 font-medium">
                <View>⚔️ Challenge anyone from your school</View>
                <View>⏱️ You have limited time to answer</View>
                <View>☠️ Wrong answers damage your health</View>
              </View>
            </View>
        ...
        {step === 4 && (
          ...
          >
            {/* The Duel Header */}
            <View className="flex items-center justify-between w-full gap-2 mb-8 px-4 flex-row">
              <View className="flex flex-col items-center flex-1">
                <View className="relative">
                  <View className={`w-16 h-16 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden border-[3px] border-indigo-500 flex items-center justify-center shadow-lg transition-transform duration-300 ${duelWon ? 'scale-110 shadow-indigo-500/50' : ''}`}>
                    {user?.user_metadata?.avatar_url && !user.user_metadata.avatar_url.includes('googleusercontent') ? (
                      <Image src={user.user_metadata.avatar_url} alt="You" className="w-full h-full object-cover" />
                    ) : (
                      <Text className="text-2xl">😎</Text>
                    )}
                  </View>
                  {duelWon && (
                    <View initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-2 -right-2 bg-yellow-400 rounded-full p-1 shadow-md">
                      <Sparkles className="w-4 h-4 text-yellow-900" />
                    </View>
                  )}
                </View>
                <Text className="text-[10px] font-black uppercase mt-3 text-indigo-600 dark:text-indigo-400 tracking-widest">{user?.user_metadata?.username || 'YOU'}</Text>
                {/* Health Bar */}
                <View className="w-16 h-2 bg-slate-200 dark:bg-slate-700 rounded-full mt-2 overflow-hidden">
                  <View className="w-full h-full bg-green-500 rounded-full" />
                </View>
              </View>
              
              <View className="flex flex-col items-center px-4">
                <View animate={duelWon ? { rotate: 180, scale: 0.5, opacity: 0 } : {}} transition={{ duration: 0.5 }}>
                  <Sword className="w-8 h-8 text-rose-500 animate-pulse mb-2" />
                </View>
                <Text className="text-[10px] font-black uppercase text-slate-400 tracking-widest">VS</Text>
              </View>

              <View className="flex flex-col items-center flex-1">
                <View className={`w-16 h-16 rounded-full bg-rose-100 dark:bg-rose-900 overflow-hidden border-[3px] border-rose-500 flex items-center justify-center shadow-lg transition-all duration-300 ${duelWon ? 'grayscale opacity-50 scale-95' : ''}`}>
                  <Text className="text-2xl">{duelWon ? '😵' : '🤖'}</Text>
                </View>
                <Text className="text-[10px] font-black uppercase mt-3 text-rose-600 dark:text-rose-400 tracking-widest">Master Bot</Text>
                {/* Health Bar */}
                <View className="w-16 h-2 bg-slate-200 dark:bg-slate-700 rounded-full mt-2 overflow-hidden">
                  <View 
                    initial={{ width: '100%' }}
            ...
            <Text className="text-center text-slate-500 dark:text-slate-400 text-sm font-medium mb-6">
            ...
            <>
              {!duelWon && (
                ...
                >
                  <Text className="text-lg font-bold text-slate-800 dark:text-slate-200 text-center mb-6">
                    Which organelle is known as the powerhouse of the cell?
                  </Text>
                  
                  <View className="space-y-3">
                    <View 
                      onPress={() => setWrongAnswer(true)}
                    ...
                    >
                      <Text className="inline-block w-8 text-indigo-500">A</Text> Nucleus
                    </View>
                    <View 
                    ...
                    >
                      <Text className="inline-block w-8 text-indigo-500">B</Text> Mitochondria
                    </View>
                    <View 
                      onPress={() => setWrongAnswer(true)}
                      className="w-full py-4 px-6 bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-2xl text-left font-bold text-slate-700 dark:text-slate-300 active:bg-rose-50 dark:active:bg-rose-900/30 active:border-rose-500 transition-colors"
                    >
                      <Text className="inline-block w-8 text-indigo-500">C</Text> Ribosome
                    </View>
                  </View>
                </View>
              )}
