...
import { Sword, Swords, Lock, CheckCircle2 } from 'lucide-react-native';
...
function TiltCard({ children, glow }: { children: React.ReactNode; glow: string }) {
  ...
  return (
    ...
    >
      <View style={{ rotateX, rotateY, transformStyle: 'preserve-3d', willChange: 'transform' }}>
...
function Bar({ v, track, fill }: { v: number; track: string; fill: string }) {
  return (
    <View style={{ width: '100%', height: 5, borderRadius: 999, background: track, overflow: 'hidden', marginTop: 6 }}>
      <View
        initial={{ width: 0 }}
...
export function CardFirstVictory({ earned, progress }: { earned: boolean; progress: number }) {
  return (
    <View style={{ paddingTop: 72, position: 'relative' }}>
      {/* Floating sword above card */}
      <View style={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 30, pointerEvents: 'none' }}>
        <View
      ...
      <TiltCard glow="rgba(59,130,246,0.20)">
        <View style={{
          ...
          {/* Shimmer */}
          <View style={{ position: 'absolute', inset: 0, overflow: 'hidden', borderRadius: 20, pointerEvents: 'none' }}>
            <View style={{
          ...
          <View style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 40, background: 'linear-gradient(to bottom,#0d1f3c,transparent)', zIndex: 10, pointerEvents: 'none', borderRadius: '20px 20px 0 0' }} />

          <View style={{ padding: '16px 18px 16px', position: 'relative', zIndex: 5 }}>
            ...
            <View style={{ fontWeight: 900, fontSize: 18, color: '#bfdbfe', letterSpacing: '-.02em' }}>First Victory</View>
            <View style={{ fontSize: 11, color: '#60a5fa99', fontWeight: 600, marginTop: 2 }}>Solve your first question</View>
            {earned
              ? <View style={{ marginTop: 8, fontSize: 11, fontWeight: 800, color: '#34d399', background: '#064e3b44', borderRadius: 8, padding: '3px 9px', display: 'inline-block' }}>✓ Unlocked</View>
              : <><Bar v={progress} track="#0d1f3c" fill="linear-gradient(90deg,#3b82f6,#60a5fa)" /><View style={{ fontSize: 10, color: '#60a5fa', fontWeight: 800, marginTop: 4 }}>{Math.round(progress * 100)}% — solve 1 question</View></>
            }
          </View>
        </View>
      </TiltCard>
...
export function CardDuelHero({ earned, progress, current }: { earned: boolean; progress: number; current: number }) {
  return (
    <View style={{ paddingTop: 72, position: 'relative' }}>
      ...
      {earned && (
        <View style={{ position: 'absolute', top: 8, left: 0, right: 0, zIndex: 28, pointerEvents: 'none' }}>
      ...
      <TiltCard glow="rgba(245,158,11,0.22)">
        <View style={{
          ...
          <View style={{ position: 'absolute', inset: 0, overflow: 'hidden', borderRadius: 20, pointerEvents: 'none' }}>
            ...
            <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '45%', background: 'linear-gradient(to top,rgba(251,146,60,.1),transparent)', pointerEvents: 'none' }} />
          </View>
          <View style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 40, background: 'linear-gradient(to bottom,#1a0a00,transparent)', zIndex: 10, pointerEvents: 'none', borderRadius: '20px 20px 0 0' }} />
          ...
          <View style={{ padding: '16px 18px 16px', position: 'relative', zIndex: 5 }}>
            ...
            <View style={{ fontWeight: 900, fontSize: 18, color: '#fde68a', letterSpacing: '-.02em' }}>Duel Hero</View>
            <View style={{ fontSize: 11, color: '#fbbf2488', fontWeight: 600, marginTop: 2 }}>Win 5 duels</View>
            {earned
              ? <View style={{ marginTop: 8, fontSize: 11, fontWeight: 800, color: '#fbbf24', background: '#78350f55', borderRadius: 8, padding: '3px 9px', display: 'inline-block' }}>✓ Unlocked</View>
              : <><Bar v={progress} track="#1a0a00" fill="linear-gradient(90deg,#d97706,#fbbf24)" /><View style={{ fontSize: 10, color: '#fbbf24', fontWeight: 800, marginTop: 4 }}>{current}/5 duel wins — {Math.round(progress * 100)}% complete</View></>
            }
          </View>
        </View>
      </TiltCard>
...
export function CardFinalBoss({ earned, progress, current }: { earned: boolean; progress: number; current: number }) {
  return (
    <View style={{ paddingTop: 84, position: 'relative' }}>

      {/* ── Demon character rising from card ── */}
      <View style={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 40, pointerEvents: 'none' }}>
        <View
          animate={{ y: [0, -12, 0] }}
      ...
      {earned && (
        <View style={{ position: 'absolute', top: 10, left: 0, right: 0, zIndex: 35, pointerEvents: 'none' }}>
      ...
      <TiltCard glow="rgba(124,58,237,0.25)">
        <View style={{
          ...
          {/* corner glow */}
          <View style={{ position: 'absolute', top: -20, right: -20, width: 90, height: 90, background: 'radial-gradient(circle,#7c3aed55,transparent 70%)', pointerEvents: 'none' }} />
          {/* emergence gradient — hides demon's lower body inside card */}
          <View style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 52, background: 'linear-gradient(to bottom,#0a0118 0%,transparent 100%)', zIndex: 10, pointerEvents: 'none', borderRadius: '20px 20px 0 0' }} />
          ...
          <View style={{ padding: '16px 18px 16px', position: 'relative', zIndex: 5 }}>
            ...
            <View style={{ fontWeight: 900, fontSize: 18, color: '#e9d5ff', letterSpacing: '-.02em' }}>Final Boss</View>
            <View style={{ fontSize: 11, color: '#a78bfa88', fontWeight: 600, marginTop: 2 }}>Solve 20 questions</View>
            {earned
              ? <View style={{ marginTop: 8, fontSize: 11, fontWeight: 800, color: '#c4b5fd', background: '#3b076466', borderRadius: 8, padding: '3px 9px', display: 'inline-block' }}>✓ Unlocked</View>
              : <><Bar v={progress} track="#2e1065" fill="linear-gradient(90deg,#6d28d9,#a78bfa)" /><View style={{ fontSize: 10, color: '#a78bfa', fontWeight: 800, marginTop: 4 }}>{current}/20 questions — {Math.round(progress * 100)}% complete</View></>
            }
          </View>
        </View>
      </TiltCard>
...
export default function AchievementCards({ userId, battlesWon, battlesAttempted }: { userId: string; battlesWon: number; battlesAttempted: number }) {
  ...
  return (
    <View style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 16 }}>
      <CardFirstVictory
