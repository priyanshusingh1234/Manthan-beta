...
const initNativePush = async (userId: string, navigate: (path: string) => void) => {
  ...
  // (Platform.OS !== 'web') can return false when loading from a remote
  // server URL, so also check for the native bridge directly.
  const isNative = (Platform.OS !== 'web') ||
    (typeof (window as any)?.Capacitor?.isNativePlatform === 'function' && (window as any).(Platform.OS !== 'web')) ||
    (typeof (window as any)?.Capacitor?.getPlatform === 'function' && (window as any).Platform.OS !== 'web');
  ...
  try {
    ...
    await PushNotifications.addListener('pushNotificationReceived', async (notification) => {
      ...
      if (data.type === 'incoming_call') {
        ...
        try {
          const LocalNotifications = { schedule: () => {}, registerActionTypes: () => {}, addListener: () => {} };
...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  // Handle Incoming Android Shares globally and immediately
  useEffect(() => {
    if (!(Platform.OS !== 'web')) return;
    let listener: any = null;
    const setupShare = async () => {
      try {
        const ShareTarget = { addListener: () => ({ remove: () => {} }) };
        listener = await ShareTarget.addListener('shareReceived', (data) => {
          console.log('[ShareTarget] shareReceived:', data);
          const text = data.text || data.url || data.title || '';
          if (text) {
            safeNavigate(`/posts?share=${encodeURIComponent(text)}`, (path) => router.push(path));
          }
        });
      } catch (e) {
