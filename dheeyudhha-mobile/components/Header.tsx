...
import { Menu, X, LogOut, User, PlusCircle, Trophy, Mail, Info, FileQuestion, BookOpen, GraduationCap, Sparkles, HelpCircle, Shield, Bell, LucideIcon, Moon, Sun, CheckSquare, Swords, Search, MessageSquare, Compass, Zap, Flame, PlaySquare, Gift } from 'lucide-react-native';
...
function StreakPill() {
  ...
  return (
    <Link href="/streaks" title="My streak">
      <Text className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-black transition-all active:scale-95 ${
        goalMet
...
const Header: React.FC<HeaderProps> = ({ isMobile = false }) => {
  ...
  const UserAvatar: React.FC = () => {
    ...
    return (
        <Text className="inline-block h-10 w-10 sm:h-11 sm:w-11 rounded-full overflow-hidden border-[3px] border-white/40 hover:border-white transition-all shadow-lg hover:shadow-white/20">
            {avatarUrl ? (
                ...
                <Text className="flex items-center justify-center h-full w-full text-xl font-bold text-indigo-700 bg-white shadow-inner flex-row">
  ...
  return (
    <>
      ...
      <View className="md:hidden sticky top-0 z-[60] bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800/80 px-4 pt-[calc(env(safe-area-inset-top)+1rem)] pb-3">
        <View className="flex items-center justify-between flex-row">
          <View className="flex items-center gap-2 flex-row">
            <Text className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">
              ...
              <>
                ...
                {user?.user_metadata?.isTeacher && (
                  <Link
                    ...
                    className="inline-flex items-center justify-center w-9 h-9 text-indigo-600 bg-indigo-100 dark:bg-indigo-900/50 rounded-full active:scale-95 flex-row"
                  >
                    <PlusCircle className="h-5 w-5" />
                  </Link>
                )}
                <Link href="/profile" className="active:scale-95 transition-transform flex items-center justify-center flex-row">
                  <Text className="inline-block h-9 w-9 rounded-full overflow-hidden border-[2px] border-slate-200 dark:border-slate-700 shadow-sm relative">
                    {(user?.user_metadata?.avatar_url || user?.user_metadata?.custom_avatar_url || user?.user_metadata?.picture) ? (
                      <Image source={{ uri: user.user_metadata.avatar_url || user?.user_metadata?.custom_avatar_url || user.user_metadata.picture }} alt="avatar" fill className="object-cover" referrerPolicy="no-referrer" />
                    ) : (
                      <Text className="flex items-center justify-center h-full w-full text-base font-bold text-indigo-600 bg-white flex-row">
        ...
        {user && (
          <View className="mt-3 overflow-x-auto scrollbar-hide flex items-center gap-1.5 pb-1 snap-x flex-row">
            {showBonusButton && (
              <View
                onPress={() => setBonusModalOpen(true)}
                className="flex-shrink-0 snap-start flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all relative bg-gradient-to-r from-indigo-500 to-purple-500 border-indigo-400 text-white shadow-[0_0_10px_rgba(99,102,241,0.4)] animate-pulse flex-row"
              >
            ...
            {[
              ...
              return (
                ...
                >
                  <View.icon className={`w-3.5 h-3.5 ${nav.href === '/streaks' ? 'text-orange-500' : nav.label === 'Search' && isFirstSearch && !active ? 'text-blue-600' : ''}`} />
                  <Text>{nav.label}</Text>
                  {nav.label === 'Search' && isFirstSearch && !active && (
                    <Text className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-red-500 border border-white dark:border-slate-900 rounded-full" />
                  )}
                </Link>
              )
            })}
      ...
      {/* Desktop Header */}
      <View className="hidden md:block relative isolate z-50">
        {/* Dynamic Main Header Background */}
        <View className="absolute inset-0 overflow-hidden -z-10 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 shadow-xl pointer-events-none rounded-b-[2rem] border-b border-white/10">
          <View className="absolute inset-0 bg-transparent bg-center opacity-20" />
          <View className="absolute inset-0 bg-gradient-to-br from-fuchsia-500/30 via-blue-500/30 to-cyan-400/30 animate-gradient-slow blur-[2px]" />

          {/* Animated glowing orbs */}
          <View className="absolute top-0 left-1/4 w-96 h-96 bg-blue-400/20 rounded-full mix-blend-overlay filter blur-3xl animate-float" />
          <View className="absolute bottom-0 right-1/4 w-96 h-96 bg-fuchsia-400/20 rounded-full mix-blend-overlay filter blur-3xl animate-float" style={{ animationDelay: '2s' }} />
        </View>

        <View className="px-4 py-3 sm:px-6 lg:px-8 mx-auto xl:max-w-screen-2xl">
          <View className="flex items-center justify-between flex-row">
            ...
            <View className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden md:flex items-center gap-2 px-6 py-2.5 bg-white/10 backdrop-blur-xl rounded-full border border-white/20 shadow-[0_8px_30px_rgb(0,0,0,0.12)] animate-slideUp overflow-hidden group hover:bg-white/15 transition-colors flex-row">
              <View className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:animate-shimmer" />
              <Text className="text-sm sm:text-base font-extrabold tracking-[0.15em] text-white drop-shadow-md">
                DHEEYUDHA
              </Text>
              {pageInfo && (
                <>
                  <Text className="text-white/40 text-[10px] sm:text-xs font-bold px-0.5">•</Text>
                  <pageInfo.icon className="w-5 h-5 text-indigo-100 animate-pulse hidden sm:block" />
                  <Text className="text-[10px] sm:text-sm font-bold text-white tracking-widest uppercase truncate max-w-[200px] lg:max-w-[400px]">
                    {pageInfo.title}
                  </Text>
                </>
              )}
              ...
              {isMounted && (
                <View className="flex items-center gap-2 flex-row">
                  {showBonusButton && (
                    <View
                      onPress={() => setBonusModalOpen(true)}
                      className="hidden md:flex items-center gap-1.5 px-3 h-10 rounded-full border text-sm font-black transition-all active:scale-95 bg-gradient-to-r from-indigo-500 to-purple-500 border-indigo-400 text-white shadow-[0_0_15px_rgba(99,102,241,0.4)] animate-pulse flex-row"
                      title="Daily Bonus"
                    >
                  ...
                  >
                    ...
                    {isFirstSearch && (
                      <Text className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-indigo-400 rounded-full border border-white shadow-sm" />
                    )}
                  </Link>
                  <Link
                    href="/streaks"
                  ...
                  <View
                    ...
                    className="hidden md:flex items-center justify-center w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 text-white transition-all active:scale-95 flex-row"
                    title="Toggle Theme"
                  >
              ...
              {user ? (
                <>
                  ...
                  {user?.user_metadata?.isTeacher && (
                    <Link
                      href="/questions/create"
                      className="lg:hidden inline-flex items-center justify-center w-11 h-11 rounded-full text-white bg-blue-500/40 border border-white/30 hover:bg-blue-500/60 transition-all shadow-md active:scale-95 flex-row"
                    >
                      <PlusCircle className="h-5 w-5" />
                    </Link>
                  )}
                  ...
                  {/* Desktop Icons */}
                  <View className="hidden lg:flex items-center gap-4 flex-row">
                    <View className="flex items-center gap-2 flex-row">
                    ...
                    <View className="relative" ref={dropdownRef}>
                      <View
                        ...
                        className="group flex items-center gap-2 outline-none flex-row"
                      >
                        <View className="relative transition-transform duration-300 group-hover:scale-105 group-hover:-translate-y-0.5">
                          <UserAvatar />
                          {/* Glow ring on hover */}
                          <View className="absolute inset-0 rounded-full bg-white opacity-0 group-hover:animate-ping mix-blend-overlay" />
                        </View>
                      </View>
                      ...
                      {dropdownOpen && typeof window !== 'undefined' && createPortal(
                        ...
                        >
                          <View className="px-5 py-5 bg-gradient-to-br from-indigo-50 dark:from-indigo-950/30 to-purple-50 dark:to-purple-950/30 border-b border-slate-100 dark:border-slate-800">
                            <Text className="text-xs font-extrabold text-indigo-500 uppercase tracking-widest mb-1 shadow-sm">Account</Text>
                            <Text className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">
                              {user.email}
                            </Text>
                          </View>

                          <View className="p-2 space-y-1 bg-white dark:bg-slate-900">
                            <Link
                              href="/profile"
                              onPress={() => setDropdownOpen(false)}
                              className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-slate-800 hover:text-indigo-600 dark:hover:text-indigo-400 hover:shadow-sm border border-transparent hover:border-indigo-100 dark:hover:border-slate-700 transition-all flex-row"
                            >
                            ...
                            <View
                              ...
                              className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm font-bold text-red-600 dark:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 hover:shadow-sm border border-transparent hover:border-red-100 dark:hover:border-red-900/50 transition-all flex-row"
                            >
        ...
        {/* Header spacer */}
        <View className="md:hidden h-[calc(env(safe-area-inset-top)+100px)]" />
        <View className="hidden md:block h-20 sm:h-24 md:h-28" />
      </View>
    </>
  );
}
