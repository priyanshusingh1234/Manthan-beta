    ...
    return (
        <View className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800/60 p-5 rounded-[2rem] shadow-sm mb-8">
            <View className="flex items-center gap-2 mb-4 px-2 flex-row">
                <Text className="font-bold text-slate-800 dark:text-slate-200 text-sm tracking-wide">SUGGESTED FOR YOU</Text>
            </View>
            ...
            <View className="flex flex-col gap-4">
                {suggestions.map((user) => {
                    ...
                    return (
                        <View key={user.id} className="flex items-center gap-3 w-full px-2 group cursor-pointer transition-transform hover:-translate-x-1 flex-row">
                            {/* Avatar */}
                            <Link href={user.isTeacher && user.username ? `/teacher/${user.username}` : user.username ? `/user/${user.username}` : '#'} className="shrink-0 relative h-10 w-10">
                                <View className="absolute inset-0 rounded-full bg-indigo-500/20 blur-sm opacity-0 group-hover:opacity-100 transition-opacity" />
                                <View className="relative h-10 w-10 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center flex-row">
                                    {user.avatar ? (
                                        <Image source={{ uri: user.avatar }} alt={user.name} className="object-cover w-full h-full" />
                                    ) : (
                                        <Text className="font-bold text-slate-400 text-sm">{user.name.charAt(0)}</Text>
                                    )}
                                </View>
                            </Link>

                            {/* Info */}
                            <View className="flex-1 min-w-0 flex justify-center flex-col h-full items-start">
                                <Link href={user.isTeacher && user.username ? `/teacher/${user.username}` : user.username ? `/user/${user.username}` : '#'} className="flex items-center gap-1.5 min-w-0 max-w-full flex-row">
                                    <Text className="font-bold text-sm text-slate-900 dark:text-slate-100 truncate flex-shrink flex-row">
                                        {user.name}
                                    </Text>
                                    {user.isTeacher && <TeacherBadge />}
                                    {Number(user.totalPoints) >= 1500 && <TopperBadge />}
                                </Link>
                                <Text className="text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate w-full mt-0.5">
                                    {user.reason}
                                </Text>
                            </View>
