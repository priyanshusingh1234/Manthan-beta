...
import { Loader2, BookOpen, ChevronRight, X, Sparkles } from 'lucide-react-native';
...
export default function DailyPlannerModal() {
  ...
  return (
    <View className="fixed inset-0 z-[999] flex flex-col justify-end sm:justify-center items-center bg-slate-900/60 backdrop-blur-sm sm:p-4 animate-in fade-in duration-300">
      <View className="bg-white dark:bg-slate-950 w-full h-[100dvh] sm:h-auto sm:max-w-md sm:rounded-[2rem] shadow-[0_-8px_30px_rgba(0,0,0,0.12)] overflow-hidden sm:border border-slate-200 dark:border-slate-800 animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-8 duration-500 ease-out relative flex flex-col">

        {/* Header / Banner */}
        <View className="relative pt-[calc(3rem+env(safe-area-inset-top,0px))] sm:pt-12 pb-6 px-6 bg-gradient-to-br from-indigo-600 via-purple-600 to-violet-700 text-center text-white">
          <View className="absolute top-[calc(1rem+env(safe-area-inset-top,0px))] right-4 z-20">
            <View
              onPress={() => {
                localStorage.setItem('dheeyudhha_daily_plan_date', new Date().getTime().toString());
                setIsOpen(false);
          ...
          <View className="w-20 h-20 mx-auto bg-white/20 backdrop-blur-md rounded-3xl mb-4 flex items-center justify-center shadow-inner rotate-3 flex-row">
            <Sparkles className="w-10 h-10 text-yellow-300" />
          </View>

          <Text className="text-2xl font-black mb-1">Hii {userName}! 👋</Text>
        ...
        {/* Content Body */}
        <View className="p-6 pb-8 sm:pb-6">
          {step === 1 && (
            <View className="space-y-4 animate-in slide-in-from-right-4 duration-500">
              <View className="grid grid-cols-2 gap-3">
                {SUBJECTS.map((sub) => (
                  ...
                  >
                    <View className={`w-12 h-12 rounded-full flex items-center justify-center text-xl bg-gradient-to-br ${sub.color} shadow-inner text-white group-hover:rotate-12 transition-transform`}>
                      {sub.emoji}
                    </View>
                    <Text className="font-bold text-slate-700 dark:text-slate-200">{sub.label}</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          {step === 2 && (
            <View className="space-y-3 animate-in slide-in-from-right-4 duration-500">
              <View
                onPress={() => setStep(1)}
                className="text-xs font-bold text-indigo-500 mb-2 flex items-center gap-1 hover:underline flex-row"
              >
                ← Back to Subjects
              </View>

              {loadingChapters ? (
                <View className="flex flex-col items-center justify-center py-12 gap-3 text-slate-500">
                  <Loader2 className="w-8 h-8 animate-spin text-indigo-500" />
                  <Text className="text-sm font-semibold animate-pulse">Finding your chapters...</Text>
                </View>
              ) : chapters.length === 0 ? (
                <View className="flex flex-col items-center justify-center py-12 gap-3 text-slate-500 text-center px-4">
                  <View className="w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center mb-2 flex-row">
                    <BookOpen className="w-6 h-6 text-slate-400" />
                  </View>
                  <Text className="text-sm font-bold text-slate-600 dark:text-slate-300">No chapters found</Text>
                  <Text className="text-xs text-slate-500 dark:text-slate-500">Looks like teachers haven't uploaded any questions for this subject in your class yet!</Text>
                </View>
              ) : (
                <View className="max-h-[50vh] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                  {chapters.map((chap, idx) => (
                    <View
                      key={idx}
                      onPress={() => handleChapterSelect(chap)}
                      className="w-full flex items-center justify-between p-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all active:scale-[0.98] group flex-row"
                    >
                      <View className="flex items-center gap-3 text-left flex-row">
                        <View className="w-8 h-8 rounded-full bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm text-indigo-600 flex-row">
                          <BookOpen className="w-4 h-4" />
                        </View>
                        <Text className="font-semibold text-slate-800 dark:text-slate-200 line-clamp-2 pr-2">{chap}</Text>
                      </View>
