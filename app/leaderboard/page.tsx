...
export default async function LeaderboardPage() {
  ...
  if (authUser) {
     const rankIndex = allStudents.findIndex(p => p.id === authUser.id);
     if (rankIndex !== -1) {
        userRankInfo = {
            rank: rankIndex + 1,
            profile: allStudents[rankIndex]
        };
     }
  }
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 dark:bg-slate-950 pb-24 relative overflow-hidden flex flex-col items-center">
      ...
      {userRankInfo && (
        <div className="fixed bottom-[calc(env(safe-area-inset-bottom)+4.5rem)] md:bottom-6 left-0 right-0 z-50 px-4 pointer-events-none flex justify-center">
          <div className="pointer-events-auto bg-slate-900/90 dark:bg-slate-800/90 backdrop-blur-md border border-slate-700 w-full max-w-sm rounded-full p-2 flex items-center justify-between shadow-2xl animate-slideUp">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-indigo-500 flex items-center justify-center text-white font-black shadow-inner border border-indigo-400">
                #{userRankInfo.rank}
              </div>
              <div className="flex flex-col">
                <span className="text-white font-bold text-sm leading-tight">Your Rank</span>
                <span className="text-slate-300 text-[10px] font-medium leading-tight">
                  {userRankInfo.profile.total_points.toLocaleString()} pts • {userRankInfo.rank <= 50 ? 'Top 50!' : 'Keep grinding!'}
                </span>
              </div>
            </div>
            {userRankInfo.rank > 50 && (
              <Link href="/questions" className="mr-1 bg-white text-slate-900 px-3 py-1.5 rounded-full text-xs font-black hover:scale-105 active:scale-95 transition-transform flex items-center gap-1 shadow-md">
