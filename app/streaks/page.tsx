...
import { Flame, Trophy, Target, Zap, Star, TrendingUp, ChevronLeft, CheckCircle, Circle, Calendar } from 'lucide-react';
...
// Generate last N calendar days for the heatmap
        ...
        <div className="relative flex items-center justify-center" style={{ width: 144, height: 144 }}>
            <svg width={144} height={144} className="absolute rotate-[-90deg]">
                ...
                <circle cx={72} cy={72} r={r} fill="none"
                    stroke="url(#streakGrad)" strokeWidth={10}
...
function Heatmap({ activeDays }: { activeDays: Set<string> }) {
    ...
    // Figure out offset so first cell starts on the right weekday
    ...
    return (
        <div className="space-y-2">
            <div className="grid grid-cols-7 gap-0.5">
                {dayNames.map((d, i) => (
                    <div key={i} className="text-[9px] font-black text-slate-400 text-center mb-0.5">{d}</div>
                ))}
                {/* Empty offset cells */}
                            ...
                            className={`aspect-square rounded-sm transition-all ${
                                isToday
                                    ? isActive
                                        ...
                                        : 'ring-2 ring-orange-300 dark:ring-orange-700 rounded-sm'
                                    : isActive
                                        ? 'bg-orange-400'
...
export default function StreakPage() {
    ...
    const dailySolved = Number(profile.daily_solve_count) || 0;
    const today = todayIST();
    const dailySolveDate = profile.daily_solve_date || null;
    const solvedToday = dailySolveDate === today ? dailySolved : 0;
    ...
    // Build active-day set from attempts (IST)
    ...
    // Weekly solve count (last 7 days)
    ...
    const lastStreakAt = profile.last_streak_at;
    const daysSinceLast = daysSince(lastStreakAt);
    let statusMsg = '';
    ...
    if (streak === 0) {
        ...
        const remaining = 2 - solvedToday;
        statusMsg = `Solve ${remaining} more question${remaining > 1 ? 's' : ''} to keep your streak alive!`;
    ...
    return (
        <div className="min-h-[100dvh] bg-slate-50 dark:bg-slate-950 pb-24">
            <main className="max-w-lg mx-auto px-4 pt-5 space-y-4">
                ...
                {/* ── Hero card ── */}
                <div className={`relative overflow-hidden bg-white dark:bg-slate-900 border rounded-3xl shadow-sm p-6 ${
                    goalMetToday
                        ? 'border-orange-200 dark:border-orange-800/40'
                        : 'border-slate-200 dark:border-slate-800'
                }`}>
                    {/* bg glow when streak active */}
                    {streak > 0 && (
                        <div className="absolute inset-0 pointer-events-none">
                            <div className="absolute top-0 right-0 w-48 h-48 bg-orange-400/5 rounded-full blur-3xl" />
                            <div className="absolute bottom-0 left-0 w-32 h-32 bg-rose-400/5 rounded-full blur-2xl" />
                        </div>
                    )}
                    ...
                    <div className="flex flex-col items-center gap-4 relative z-10">
                        ...
                        <div className="w-full space-y-2">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                                Today's Goal — {solvedToday}/2 questions
                            </p>
                            ...
                            <div key={day} className="flex-1 flex flex-col items-center gap-1">
                                <div className="w-full flex items-end justify-center" style={{ height: 80 }}>
                                    <div
                                        ...
                                        style={{ height: `${Math.max((count / maxWeekly) * 80, count > 0 ? 8 : 4)}px` }}
                                    />
                                </div>
                                <span className={`text-[9px] font-black ${day === today ? 'text-orange-500' : 'text-slate-400'}`}>
                                    {label}
                                </span>
                                {count > 0 && (
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm p-5 space-y-3">
                    ...
                    <div className="space-y-2">
                        {[
                            ...
                            return (
                                <div key={target} className={`flex items-center gap-3 p-3 rounded-2xl border transition-all ${
                                    done
                                        ? 'border-orange-200 dark:border-orange-800/40 bg-orange-50/50 dark:bg-orange-900/5'
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {!done && (
                                            <div className="mt-1 h-1.5 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                                                <div
                                                    className="h-full rounded-full bg-gradient-to-r from-orange-400 to-rose-500 transition-all duration-1000"
