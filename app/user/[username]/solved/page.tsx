...
export default async function UserSolvedQuestionsPage({ params }: Props) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
                <div className="text-center">
                    <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-2">User Not Found</h2>
                    <p className="text-slate-500 mb-6">The student @{username} could not be located.</p>
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20 pt-6 sm:pt-10">
            <div className="max-w-3xl mx-auto px-4 sm:px-6">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 mb-8 sm:mb-12 text-center sm:text-left">
                    <Link href={`/user/${username}`} className="w-full sm:w-auto flex justify-center sm:block">
                        <button className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm hover:shadow-md hover:-translate-x-1 group">
                    ...
                    <div className="flex-1">
                        <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center justify-center sm:justify-start gap-2.5 sm:gap-3 capitalize">
                            <Trophy className="w-6 h-6 sm:w-8 sm:h-8 text-amber-500 shrink-0" />
                            <span>{username}'s Achievements</span>
                        </h1>
                ...
                <div className="space-y-4 sm:space-y-6 px-1 sm:px-0">
                    {displayQuestions.length === 0 ? (
                        <div className="py-20 sm:py-24 text-center bg-white dark:bg-slate-900 rounded-3xl sm:rounded-[3rem] border-2 border-dashed border-slate-100 dark:border-slate-800 shadow-sm transition-all">
                             <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-50 dark:bg-slate-800/50 rounded-full flex items-center justify-center mx-auto mb-5 sm:mb-6">
                                <Star className="w-8 h-8 sm:w-10 sm:h-10 text-slate-300 dark:text-slate-600" />
                             </div>
                             <h3 className="text-lg sm:text-xl font-black text-slate-800 dark:text-white mb-2 underline decoration-indigo-500/30">Quiet on the front</h3>
                             <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto text-[13px] sm:text-sm font-medium">No questions solved yet. Check back once @{username} masters some topics!</p>
                        </div>
                    ) : (
                        displayQuestions.map((q) => (
                            <div key={q.id} className="transition-transform active:scale-[0.98] duration-200">
                                <QuestionCard q={q} />
                            </div>
                        ))
                    )}
