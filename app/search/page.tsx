...
import { Search, User, MessageSquare, FileQuestion, Loader2, ArrowLeft, X } from 'lucide-react';
...
function SearchPageContent() {
    ...
    const UserResult = ({ user }: { user: any }) => (
        <Link 
            ...
            className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-indigo-900/50 transition-all group"
        >
            <div className="relative w-12 h-12 rounded-full overflow-hidden bg-indigo-100 dark:bg-indigo-900/30 border-2 border-white dark:border-slate-800 shrink-0">
                {user.avatar_url ? (
            ...
            <div className="flex-1 min-w-0">
                <h4 className="font-bold text-slate-900 dark:text-slate-100 truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                    {user.full_name}
                </h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">@{user.username}</p>
                {user.school && (
                    <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-tight mt-1">{user.school}</p>
                )}
            </div>
        </Link>
    );
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-slate-950 pb-20">
            ...
            <div className="sticky top-0 z-50 bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800/80 pt-[env(safe-area-inset-top)]">
                ...
                {results && (
                    <div className="max-w-4xl mx-auto flex items-center px-2 pb-2 overflow-x-auto scrollbar-hide">
                        {[
                            ...
                            return (
                                ...
                                >
                                    {isTabActive && (
                                        <div className="absolute inset-0 bg-indigo-500/5 rounded-xl animate-in fade-in" />
                                    )}
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4 text-slate-400">
                        ...
                        <p className="font-bold tracking-wide uppercase text-xs">Hunting for results...</p>
                    </div>
                ) : !results ? (
                    <div className="text-center py-20 flex flex-col items-center gap-6">
                        ...
                        <div className="space-y-2">
                            <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100">Find anything on Dheeyudha</h3>
                    ...
                    <div className="space-y-12">
                        ...
                        {results.users.length === 0 && results.posts.length === 0 && results.questions.length === 0 && (
                            <div className="text-center py-20">
                                <p className="text-slate-500 font-bold italic">No results found for "{query}"</p>
                                <button 
                                    onClick={clearSearch}
...
export default function SearchPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-white dark:bg-slate-950 flex items-center justify-center text-slate-400 font-bold italic">Loading search environment...</div>}>
            <SearchPageContent />
        </Suspense>
    );
}
